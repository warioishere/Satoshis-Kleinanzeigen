...
class UAC_Dokan_Dashboard {
    ...
    public function load_template($query_vars) {
        if (isset($query_vars['auth-connector'])) {
            require_once UNIFIED_AUTH_CONNECTOR_PATH . 'templates/dashboard-auth-connector.php';
        }
    }
    ...
    public function ajax_link_nostr() {
        ...
        if ($sync_profile && class_exists('SK_Lite') && function_exists('sk_is_user_seller') && sk_is_user_seller($user_id)) {
            $profile_sync = new UAC_Nostr_Profile_Sync();
