# Treuhand nach Regelwerk 1.2: was das Modul `sk_escrow` dafür können muss

Stand 17.09.2026. Regelwerk: `docs/treuhand-regelwerk-v1.html`, Fassung 1.2 (§-Verweise unten).

**Fassung 1.2 (Red-Team-Ergebnis, umgesetzt):** Die vier Maschen, die 1.1 zuließ, und was dagegen steht — (1) Ziegelstein-Rücksendung: bleibt als Restrisiko, begrenzt durch Stufenlimits, Rückstufung schon beim ersten Vorfall, Rückgewicht ≥ 90 % des Hinwegs und die Verkäufer-Option „nur Käufer ab Stufe 1/2“ (`weo_min_buyer_tier`); (2) Kulanzfonds als Geldautomat: erst Stufe 2, 25 %/250k, Vorfall beim Gegenkonto erst mit Auszahlung, Kontomuster-Prüfung durch SK; (3) Stufen für 9'000 Sats: `Rules::completed_trades()` zählt nur Handel ≥ 20k mit je einer eigenständigen, selbst vertrauenswürdigen Gegenseite (`trusted_counterparty`: WoT oder ≥ 3 eigene Handel), nie zwischen Konten mit gemeinsamer Bitcoin-Adresse; (4) Vorfälle als Waffe: nur ausgezahlte Anträge zählen. Dazu: zu leichtes Paket (§10) = nicht versendet (volle Erstattung ohne Rücksendung, `Deadlines::weight_check()` eskaliert), Rücksendung 14 Tage weder zugestellt noch verweigert = Käufer gewinnt (steht im Regelwerk, Claude bekommt `return_overdue_14_days` und `return_weight_matches` als Tatsachen). Ist-Stand des Moduls: Escrow-Zeile in `sk_lightning_payments` mit `context = 'escrow'`, Status `requested → pending → confirmed → delivered | refunded | disputed | expired`, PSBT-Fluss über die externe API mit drei Schlüsseln (Käufer, Verkäufer, Marktplatz; jede Bewegung braucht zwei), Dispute heute manuell unter WP-Admin › Treuhand › Disputes. Nichts davon ist live.
...
- **Fonds-Anträge gebaut (17.09.2026, dev):** Nach Abschluss zeigt die Karte der Seite, die die Entscheidung in `pool_claim_eligible` nennt, den Antrag (Betrag = min(50 % Preis, 500'000, Fondsstand), ab Stufe 1, einmal je 12 Monate über User-Meta `sk_escrow_claim_at`, einer je Handel). `Dispute::claim()` speichert `metadata.escrow.dispute.claim` mit Lightning-Adresse oder Rechnung und zählt den Vorfall beim Gegenkonto. Der Admin zahlt **von Hand** aus der Plattform-Wallet (es gibt keinen Plattform-Zahlweg im Code; das ist gewollt, kein Automatismus bewegt Geld) und trägt „bezahlt“ mit Referenz ein, was `claim_payout` negativ im Fonds bucht; „abgelehnt“ mit Grund. Liste unten auf der Dispute-Seite.
- **Offen:** Tracking-Dienst (Aggregator wie Ship24/17TRACK; bis dahin manuell); Fondsstand auf der Seite anzeigen (Shortcode erst bei Freischaltung); ein echter Testlauf gegen die Anthropic-API mit einem realen Fall vor der Freischaltung; Rechtsprüfung.
