...
class SK_Follow_Store_Send_Updates extends SkBackgroundProcesses {
     ...
     * @var string
     */
    protected $action = 'SK_Follow_Store_Send_Updates';
    ...
    public function task( $args ) {
        ...
        foreach ( $followers as $follower_id ) {
            ...
            if ( ! empty( $vendors_ids ) ) {
                ...
                if ( ! empty( $vendors ) ) {
                    do_action( 'sk_follow_store_send_update_email', $follower, $vendors );
                }
            }
        }
    ...
    private function get_vendor_new_coupons( $follower, $vendor_id, $args ) {
        ...
        if ( $new_coupons->have_posts() ) {
            foreach ( $new_coupons->posts as $post ) {
                ...
                if ( sk_follower_can_user_coupon( $customer_emails, $coupon ) ) {
                    $coupons[] = $coupon;
                }
            }
        }
