...
class DPS_PayPal_Standard_Subscriptions {
    ...
    public static function paypal_settings_args( $form_fields ) {
        ...
        $form_fields['email']['description'] .= ' </p><p class="description">' . __( 'It is <strong>strongly recommended you do not change this email address</strong> if you have active subscriptions with PayPal. Doing so can break existing subscriptions.', 'sk' );

        $form_fields += array(
            'api_credentials' => array(
                'title'       => __( 'API Credentials', 'sk' ),
                'type'        => 'title',
                'description' => sprintf( __( 'Enter your PayPal API credentials to unlock subscription suspension and cancellation features. %1$sLearn More &raquo;%2$s', 'sk' ), '<a href="http://docs.woothemes.com/document/store-manager-guide/#extrapaypalconfigurationsteps" target="_blank" tabindex="-1">', '</a>' ),
                'default'     => '',
            ),
            'api_username' => array(
                'title'       => __( 'API Username', 'sk' ),
                'type'        => 'text',
                'description' => '',
                'default'     => '',
            ),
            'api_password' => array(
                'title'       => __( 'API Password', 'sk' ),
                'type'        => 'text',
                'description' => '',
                'default'     => '',
            ),
            'api_signature' => array(
                'title'       => __( 'API Signature', 'sk' ),
                'type'        => 'text',
                'description' => '',
                'default'     => '',
            ),
    ...
    public static function paypal_standard_subscription_args( $paypal_args ) {
        ...
        if ( self::has_subscription() ) {
            throw new Exception( __( 'Sorry, with paypal you can\'t switch subscription plan.', 'sk' ) );
        }

        $subscription = sk()->subscription->get( $product['product_id'] );
        ...
        if ( $subscription->is_recurring() ) {
            ...
            if ( ! Helper::has_used_trial_pack( get_current_user_id() ) && $subscription->is_trial() ) {
                ...
                if ( $subscription_trial_lenth > $max_trial_length_in_days ) {
                    throw new Exception( __( 'Trial subscription can\'t be more than 720 days for PayPal', 'sk' ) );
                }
            ...
            if ( 0 === $subscription_installments ) {
                ...
                throw new Exception( __( 'Invalid subscription length', 'sk' ) );
            }
        }
    ...
    public static function process_paypal_ipn_request( $transaction_details ) {
        ...
        $vendor_subscription = sk()->subscription->get( $product['product_id'] );
        ...
        switch ( $transaction_details['txn_type'] ) {
            case 'subscr_signup':
                ...
                if ( $vendor_subscription->is_trial() ) {
                    ...
                    $order->add_order_note( sprintf( __( 'IPN subscription trial activated. Subscription ID: %s', 'sk' ), $subscription_id ) );

                    // store trial information as user meta
                    update_user_meta( $customer_id, '_sk_subscription_is_on_trial', 'yes' );
                    ...
                    if ( $time ) {
                        update_user_meta( $customer_id, '_sk_subscription_trial_until', $time->format( 'Y-m-d H:i:s' ) );
                    }
                } else {
                    // translators: 1) PayPal Subscription ID
                    $order->add_order_note( sprintf( __( 'IPN subscription activated. Subscription ID: %s', 'sk' ), $subscription_id ) );
                }
                $order->save();
            ...
            case 'subscr_payment':
                if ( 'completed' === strtolower( $transaction_details['payment_status'] ) ) {
                    ...
                    $is_renewal    = ! empty( $order->get_meta( '_sk_paypal_payment_capture_id' ) );
                    $renewal_order = null;
                    ...
                    if ( $is_renewal ) {
                        ...
                        if ( is_wp_error( $renewal_order ) ) {
                            sk_log( '[PayPal] Create Renewal Order Failed. Error: ' . $renewal_order->get_error_message() );
                            return;
                        }

                        // translators: %s: order number.
                        $order_number = sprintf( _x( '#%s', 'hash before order number', 'sk' ), $renewal_order->get_order_number() );

                        // translators: %s: order number.
                        $subscription_order_number = sprintf( _x( '#%s', 'hash before order number', 'sk' ), $order->get_order_number() );

                        // translators: placeholder is order ID
                        $order->add_order_note( sprintf( __( 'Order %s created to record renewal.', 'sk' ), sprintf( '<a href="%s">%s</a> ', esc_url( Helper::get_edit_post_link( $renewal_order->get_id() ) ), $order_number ) ) );

                        // add order note on renewal order
                        // translators: 1) subscription order number
                        $renewal_order->add_order_note( sprintf( __( 'Order created to record renewal subscription for %s.', 'sk' ), sprintf( '<a href="%s">%s</a> ', esc_url( Helper::get_edit_post_link( $order->get_id() ) ), $subscription_order_number ) ) );
                    ...
                    $order->add_order_note( sprintf( __( 'IPN subscription payment completed. Transaction ID: %s', 'sk' ), $transaction_details['txn_id'] ) );

                    // Record payment capture id
                    $order->update_meta_data( '_sk_paypal_payment_capture_id', $transaction_details['txn_id'] );
                    $order->save();
                    ...
                    // Subscription Payment completed
                    $order->add_order_note( __( 'IPN subscription payment failed.', 'sk' ) );
            ...
            case 'subscr_cancel':
                ...
                $order->add_order_note( __( 'IPN subscription cancelled.', 'sk' ) );
            ...
            case 'subscr_eot':
                ...
                $order->add_order_note( __( 'IPN subscription end-of-term for order.', 'sk' ) );
            ...
            case 'subscr_failed':
                ...
                // Subscription Payment completed
                $order->add_order_note( __( 'IPN subscription payment failure.', 'sk' ) );
    ...
    public static function cancel_subscription_with_paypal( $order_id, $user_id ) {
        ...
        if ( $response ) {
            ...
            $order->add_order_note( __( 'Subscription cancelled with PayPal', 'sk' ) );
        }
    }
    ...
    public static function change_subscription_status( $profile_id, $new_status ) {
        switch ( $new_status ) {
            case 'Cancel':
                $new_status_string = __( 'cancelled', 'sk' );
                break;
            case 'Suspend':
                $new_status_string = __( 'suspended', 'sk' );
                break;
            case 'Reactivate':
                $new_status_string = __( 'reactivated', 'sk' );
                break;
        }
        ...
        $request = wp_remote_post(
            self::$api_endpoint, array(
                ...
                'body'      => array(
                    ...
                    'ACTION'    => $new_status,
                    'NOTE'      => sprintf( __( 'Subscription %1$s at %2$s', 'sk' ), $new_status_string, get_bloginfo( 'name' ) ),
                ),
            )
        );
    ...
    public static function subscription_paypal_credential_verify() {
        ...
        if ( $paypal_settings && ! isset( $paypal_settings['debug'] ) ) {
            ...
            function sk_paypal_credential_error() {
                ?>
                <div id="message" class="error notice is-dismissible">
                    <p><?php esc_html_e( 'Your Paypal Credentials are not complete', 'sk' ); ?>.</p>
                    <button type="button" class="notice-dismiss">
                        <span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'sk' ); ?></span>
                    </button>
                </div>
                <?php
            }
            add_action( 'admin_notices', 'sk_paypal_credential_error' );
        } else {
