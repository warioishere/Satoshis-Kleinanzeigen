...
class Login {
	...
	public function __construct() {
		...
		$this->callback_url           = get_option( SK_AUTH_LNURL_PREFIX . '-callback-url' );
		$this->redirect_url           = get_option( SK_AUTH_LNURL_PREFIX . '-redirect-url' );
	...
	public function lnurl_auth_loginform_bodyclass( $classes ) {
		global $errors;
		$o = get_option( SK_AUTH_LNURL_PREFIX . '-login-options' );
		...
		isset( $_GET['key'] )
			...
			if ( ! empty( $signed ) ) {
				...
				if ( empty( $user_exists ) ) {
					...
					$prefix        = get_option( SK_AUTH_LNURL_PREFIX . '-usercreation-prefix' );
					...
					$usercration_roles = get_option( SK_AUTH_LNURL_PREFIX . '-usercreation-roles' );
				...
				lnurl_auth()->Transients->set( $k1, true, $user_id );
	...
	public function create_lnurl( $lnurl = false ) {
		...
		if ( empty( $lnurl ) ) {
			...
			for ( $i = 0; $i <= 99; $i++ ) {
				...
				$k1 = $this->get_new_key();
				if ( empty( lnurl_auth()->Transients->get( $k1 ) ) ) {
					break;
				}
