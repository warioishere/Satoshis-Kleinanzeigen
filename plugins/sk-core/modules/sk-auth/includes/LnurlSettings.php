...
class Settings {
	...
	public function __construct() {
		...
		$this->menu_slug      = SK_AUTH_LNURL_PREFIX;
		$this->options_prefix = SK_AUTH_LNURL_PREFIX . '-settings';
		$this->settings_group = SK_AUTH_LNURL_PREFIX . '-settings-group';
		$this->display_settings = array(); // Will be populated on init hook
	}

	/**
	 * Initialize display settings with translations.
	 * This is called on the 'init' hook to ensure translations are loaded.
	 *
	 * @since 1.0.0
	 */
	 ...
	 */
	public function run() {
		// Initialize display settings on init hook (after translations are loaded)
		add_action( 'init', array( $this, 'init_display_settings' ), 11 );

		// Plugin action links not needed — settings are in SK PHP Dashboard.
	...
	public function register_settings() {
		foreach ( $this->display_settings as $name => $data ) {
			...
			if ( 'string' === $data['type'] ) {
				if ( SK_AUTH_LNURL_PREFIX . '-usercreation-prefix' === $name ) {
	...
	public function render_settings_page() {
		// Admin assets handled by AuthSettings;
		...
		// attach logo
		$logo = Logo::create( SK_AUTH_PATH . '/assets/qr-logo-placeholder.png' )
	...
	public function user_list_fill_columns( $value, $column_name, $user_id ) {
		if ( 'lnurl-auth' === $column_name ) {
			$lnurl_field_value = get_user_meta( $user_id, 'lnurl-auth-bjm-id', true );
	...
	public function user_edit_update_settings_field( $user_id ) {
		...
		if ( isset( $_POST[ 'lnurl-auth-bjm-id' ] ) ) {
			update_user_meta( $user_id, 'lnurl-auth-bjm-id', sanitize_key( $_POST[ 'lnurl-auth-bjm-id' ] ) );
		}
	}
}
