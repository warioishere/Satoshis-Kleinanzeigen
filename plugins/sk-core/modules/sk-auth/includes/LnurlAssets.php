...
class Assets {
	...
	public function lnurl_auth_enqueue_scripts_styles() {
		$js = Helpers::minimize_javascript( $this->lnurl_auth_javascript() );
		...
		$css = Helpers::minimize_css( $this->lnurl_auth_css() );
	...
	public function lnurl_auth_enqueue_admin_scripts_styles() {
		$css = Helpers::minimize_css( $this->lnurl_auth_admin_css() );
		wp_deregister_style( 'lnurl-auth' ); // prevent from double enqueuing
		wp_register_style( 'lnurl-auth', '', array(), md5( $css ) );
		wp_enqueue_style( 'lnurl-auth' );
		wp_add_inline_style( 'lnurl-auth', $css );
	...
	public function lnurl_auth_javascript() {
		...
		Element.prototype.lnurlAuthReset = function() {
			...
			let seconds = this.querySelector("." + lnurlAuthElementClass + "-timer-seconds");
                        ...
                        let reinit = this.querySelector("." + lnurlAuthElementClass + "-reinit");
                        qrcode.innerHTML = "";
                        permalink.innerHTML = "";
                        minutes.innerText = "5";
                        ...
                        if (reinit) {
                                reinit.hidden = false;
                                reinit.removeAttribute("aria-hidden");
                                reinit.removeAttribute("tabindex");
                        }
                        this.classList.remove(`⚡️`);
                        this.classList.remove(`notification`);
                        this.classList.remove(`lnurl-auth-processing`);
							...
							if (response.status === `Waiting`) {
								// console.log("Await authentication:", element);
                                                        ...
                                                        if (response.status === `Signed`) {
                                                                ...
                                                                clearInterval(countdown);
                                                                const messageWrapper = element.querySelector("." + lnurlAuthElementClass + "-message");
                                                                const reinitButton = element.querySelector("." + lnurlAuthElementClass + "-reinit");
                                                                element.classList.add("notification");
                                                                element.classList.add("lnurl-auth-processing");
                                                                if (reinitButton) {
                                                                        reinitButton.hidden = true;
                                                                        reinitButton.setAttribute("aria-hidden", "true");
                                                                        reinitButton.setAttribute("tabindex", "-1");
                                                                }
                                                                if (messageWrapper) {
                                                                        messageWrapper.innerHTML = `
                                                                                <div class="${lnurlAuthElementClass}-loading" role="status" aria-live="polite">
                                                                                        <div class="${lnurlAuthElementClass}-loading-spinner" aria-hidden="true"></div>
                                                                                        <p class="${lnurlAuthElementClass}-loading-text">${lnurlAuthProcessingMessage}</p>
                                                                                </div>
                                                                        `;
                                                                }
                                                                const urlSearchParams = new URLSearchParams(window.location.search);
                                                                const params = Object.fromEntries(urlSearchParams.entries());
                                                                const redirectTarget = params.redirect_to || (element.dataset.redirect ? element.dataset.redirect : response.redirect);
	...
	public function lnurl_auth_css() {
		...
		.lnurl-auth-reinit { margin-top: 1.25em; background: inherit; appearance: none; border: 1px solid; padding: 0.3em 0.5em; color: inherit; cursor: pointer; }
                ...
                .lnurl-auth-processing .lnurl-auth-message-scroll-wrapper {
                        display: flex;
                        align-items: center;
                ...
                .lnurl-auth-processing .lnurl-auth-reinit { display: none; }
                .lnurl-auth-loading { display: flex; flex-direction: column; align-items: center; gap: 0.75em; text-align: center; }
                .lnurl-auth-loading-spinner { width: 2.75em; height: 2.75em; border-radius: 999px; border: 3px solid currentColor; border-top-color: transparent; animation: lnurl_auth_spinner 0.85s linear infinite; }
                .lnurl-auth-loading-text { font-size: 0.95em; opacity: 0.8; }
