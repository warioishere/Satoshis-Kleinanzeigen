<?php
namespace SK\Modules\ProductAdvertisement\REST;
...
class AdvertisementController extends WP_REST_Controller {
    ...
    public function prepare_item_for_response( $item, $request ) {
        $data = [
            ...
            'added'         => $item['added'] > 0 ? sk_format_date( $item['added'] ) : '',
        ];

        $response = rest_ensure_response( $data );
        $response->add_links( $this->prepare_links( $item, $request ) );

        return apply_filters( 'sk_rest_prepare_advertisement_object', $response, $item, $request );
    }
