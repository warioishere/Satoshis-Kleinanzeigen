...
class Manager {
    ...
    public function __construct() {
        global $wpdb;
        $this->table = "{$wpdb->prefix}sk_advertised_products";
    }

    /**
     * This method will return data from sk_advertised_products table
     *
     *
     * @param array $args
     *
    ...
    public function insert( $args = [] ) {
        ...
        if ( isset( $args['expires_after_days'] ) ) {
            ...
            if ( -1 === $expires_after_days ) {
                ...
                $expires_after_days = sk_current_datetime()->modify( "+{$expires_after_days} days" )->getTimestamp();
            }
    ...
    public function delete( $id = 0 ) {
        ...
        do_action( 'sk_before_deleting_product_advertisement', $id );
    ...
    public function batch_delete( $ids = [] ) {
        ...
        do_action( 'sk_before_batch_delete_product_advertisement', $ids );
    ...
    public function delete_advertisement_by_product_id( $product_id ) {
        ...
        do_action( 'sk_before_deleting_product_advertisement', $advertisement_data['id'] );
    ...
    public function batch_expire( $ids = [] ) {
        ...
        do_action( 'sk_before_batch_expire_product_advertisement', $ids );
    ...
    public function expire_advertisement_by_date( $timestamp = null ) {
        ...
        if ( false === $updated ) {
            ...
            sk_log( '[SK Product Advertisement] Error while expiring advertisements. Error: ' . $wpdb->last_error );
    ...
    public function get_stores( $args = [] ) {
        ...
        $data = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT $fields FROM {$wpdb->prefix}sk_advertised_products as featured $join WHERE %d=%d $where $groupby $orderby $limits", //phpcs:ignore
                $query_args
            ),
            ARRAY_A
        );
