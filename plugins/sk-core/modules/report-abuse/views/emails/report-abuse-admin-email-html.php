...
<div id="sk-report-abuse-email">
    <?php
        ...
        printf( '<p><strong>%s:</strong> %s</p>', esc_html__( 'Reason', 'sk' ), esc_html( $data['reason'] ) );

        if ( $data['description'] ) {
            printf( '<p><strong>%s:</strong> %s</p>', esc_html__( 'Description', 'sk' ), esc_html( $data['description'] ) );
        }
        ...
        printf(
            '<p>%s</p>',
            esc_html__( 'You can draft or remove the product or you can ignore this email if you think the product is OK.', 'sk' )
        );
        ?>
</div>
<?php
