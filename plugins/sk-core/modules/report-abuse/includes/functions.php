 ...
 * Module class object
 *
 * @return \SK\Modules\ReportAbuse\Module
 */
function sk_report_abuse() {
    return sk_ext()->module->report_abuse;
}

/**
 * Include Report Abuse template
 *
 *
 * @param string $name
 * @param array  $args
 ...
 * @return void
 */
function sk_report_abuse_template( $name, $args = [] ) {
    sk_get_template( "$name.php", $args, SK_REPORT_ABUSE_VIEWS, trailingslashit( SK_REPORT_ABUSE_VIEWS ) );
}
 ...
 * @return array
 */
function sk_report_abuse_create_report( $args ) {
    global $wpdb;
    ...
    $option = get_option( 'sk_report_abuse', [] );
    ...
    $inserted = $wpdb->insert(
        $wpdb->prefix . 'sk_report_abuse_reports',
        $report,
        $placeholders
    );
    ...
    $report = $wpdb->get_row(
        "select * from {$wpdb->prefix}sk_report_abuse_reports where id = {$wpdb->insert_id}"
    );
     ...
     */
    do_action( 'sk_report_abuse_created_report', $report, $product, $vendor, $customer );
 ...
 * @return array
 */
function sk_report_abuse_get_option() {
    $option = get_option( 'sk_report_abuse', [] );
    ...
    foreach ( $option['abuse_reasons'] as $key => $status ) {
        $option['abuse_reasons'][$key]['value'] = apply_filters('sk_pro_abuse_report_reason', $status['value'] );
    }
...
function sk_report_abuse_report_form( $args = [], $echo = false ) {
    ...
    if ( $echo ) {
        sk_report_abuse_template( 'report-form', $args );
    } else {
        ob_start();
        sk_report_abuse_template( 'report-form', $args );
        return ob_get_clean();
 ...
 */
function sk_report_abuse_get_reports( $args = [] ) {
    global $wpdb;
    ...
    if ( ! $args['count'] ) {
        $sql = 'select * from ' . $wpdb->prefix . 'sk_report_abuse_reports where 1=1';
    } else {
        $sql = 'select count(*) from ' . $wpdb->prefix . 'sk_report_abuse_reports where 1=1';
    }
    ...
    if ( $args['count'] && false === $reports  ) {
        ...
        foreach ( $results as $i => $result ) {
            ...
            $vendor = sk_get_vendor( $result->vendor_id );
            $reports[ $i ]['vendor'] = [
                'id'        => $vendor->get_id(),
