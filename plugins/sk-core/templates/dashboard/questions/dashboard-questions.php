<?php
/**
 * Questions about the vendor's listings.
 *
 * Same shell as the verification page: sk-review-page-header for the
 * heading, sk-settings-form as the frame and sk-settings-section per card,
 * because the dark field styling hangs off sk-settings-form.
 *
 * Variables come from QuestionsPage::view_data(); this file only renders.
 *
...
use SK\Core\Dashboard\Modules\QuestionsPage;

$sk_url  = function_exists( 'sk_get_navigation_url' ) ? sk_get_navigation_url( 'fragen' ) : home_url( '/dashboard/fragen/' );
$sk_tabs = [
...
<div class="sk-dashboard-wrap">
    ...
    <div class="sk-dashboard-content sk-dashboard-content--questions">
        <?php do_action( 'sk_dashboard_content_inside_before' ); ?>

        <div class="sk-review-page-header">
            <h2><i class="fas fa-circle-question"></i> <?php esc_html_e( 'Fragen', 'sk-core' ); ?></h2>
        </div>
        ...
        <ul class="sk-questions-tabs">
            <?php foreach ( $sk_tabs as $sk_key => $sk_label ) : ?>
        ...
        <?php foreach ( $rows as $sk_row ) :
            ...
            <form method="post" action="<?php echo esc_url( $sk_url ); ?>" class="sk-settings-form sk-question">
                <?php wp_nonce_field( QuestionsPage::NONCE, 'sk_q_nonce' ); ?>
