<?php
/**
 * SK Update Customer to Vendor Template.
 *
 *
 * @var int    $user_id
...
<h2><?php esc_html_e( 'Update account to Vendor', 'sk-core' ); ?></h2>
<form method="post" action="" class="update-customer-to-vendor register">
    <div class="sk-become-seller">
         ...
         *
         */
        do_action( 'sk_after_seller_migration_fields' );
        ...
        if ( $show_toc === 'on' && ! empty( $toc_page_id ) ) {
            ...
            <p class="form-row form-group form-row-wide">
                ...
                <label style="display: inline" for="tc_agree">
                    <?php
                    $tc_link = sprintf( '<a target="_blank" href="%1$s">%2$s</a>', esc_url( $toc_page_url ), __( 'Terms &amp; Conditions', 'sk-core' ) );
                    // translators: 1. Terms and conditions of agreement link.
                    printf( __( 'I have read and agree to the %1$s.', 'sk-core' ), $tc_link );
                    ?>
                </label>
            </p>
        <?php } ?>
        ...
        <p class="form-row">
            <?php wp_nonce_field( 'account_migration', 'sk_nonce' ); ?>
            <input type="hidden" name="user_id" value="<?php echo esc_attr( $user_id ); ?>">
            <input type="submit" class="sk-btn sk-btn-default" name="sk_migration" value="<?php esc_attr_e( 'Become a Vendor', 'sk-core' ); ?>" />
        </p>
    </div>
</form>
...
<script>
    (function($) {
        ...
        $( 'form.update-customer-to-vendor.register input#shop-phone' ).on( 'keydown', sk_sanitize_phone_number );
    })(jQuery);
</script>