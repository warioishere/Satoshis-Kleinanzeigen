<?php
/**
 * SK Dashboard Product Listing status filter
 * Template
 *
 *
 *
 ...
 */
?>
<ul class="sk-listing-filter sk-left subsubsub">
    <li<?php echo $status_class === 'all' ? ' class="active"' : ''; ?>>
        <a href="<?php echo esc_url( $permalink ); ?>">
            <?php
            // translators: 1) All product count
            printf( esc_html__( 'All (%s)', 'sk-core' ), esc_html( number_format_i18n( $post_counts->total ) ) );
            ?>
        </a>
    </li>
    <?php
    ...
    <?php if ( $instock_counts ) : ?>
        <li<?php echo $status_class === 'instock' ? ' class="active"' : ''; ?>>
            <a href="<?php echo esc_url( add_query_arg( [ 'post_status' => 'instock', '_product_listing_filter_nonce' => $nonce ], $permalink ) ); ?>"><?php esc_html_e( 'In stock', 'sk-core' ) . ' (' . esc_html( $instock_counts ) . ')'; ?></a> <?php // phpcs:ignore ?>
        </li>
    <?php endif; ?>

    <?php if ( $outofstock_counts ) : ?>
        <li<?php echo $status_class === 'outofstock' ? ' class="active"' : ''; ?>>
            <a href="<?php echo esc_url( add_query_arg( [ 'post_status' => 'outofstock', '_product_listing_filter_nonce' => $nonce ], $permalink ) ); ?>"><?php esc_html_e( 'Out of stock', 'sk-core' ) . ' (' . esc_html( $outofstock_counts ) . ')'; ?></a> <?php // phpcs:ignore ?>
        </li>
    <?php endif; ?>
