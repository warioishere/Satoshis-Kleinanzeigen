<li class="product-attribute-list <?php echo esc_attr( implode( ' ', $metabox_class ) ); ?>" data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>">
    <div class="sk-product-attribute-heading">
        <span><i class="fas fa-bars" aria-hidden="true"></i>&nbsp;&nbsp;<strong><?php echo ! empty( $attribute_label ) ? esc_html( $attribute_label ) : esc_html__( 'Attribute Name', 'sk' ); ?></strong></span>
        <a href="#" class="sk-product-remove-attribute"><?php esc_html_e( 'Remove', 'sk' ); ?></a>
    ...
    <div class="sk-product-attribute-item sk-clearfix sk-hide">
        <div class="content-half-part">
            <label class="form-label" for=""><?php esc_html_e( 'Name', 'sk' ); ?></label>
			...
			<label class="checkbox-item form-label">
				<input type="checkbox" <?php checked( $attribute['is_visible'], 1 ); ?> name="attribute_visibility[<?php echo $i; ?>]" value="1" /> <?php esc_html_e( 'Visible on the product page', 'sk' ); ?>
			</label>

			<label class="checkbox-item form-label show_if_variable show_if_variable-subscription">
				<input type="checkbox" <?php checked( $attribute['is_variation'], 1 ); ?> name="attribute_variation[<?php echo $i; ?>]" value="1" /> <?php esc_html_e( 'Used for variations', 'sk' ); ?>
			</label>
        </div>

        <div class="content-half-part sk-attribute-values">
            <label for="" class="form-label"><?php esc_html_e( 'Value(s)', 'sk' ); ?></label>
			<?php if ( $attribute['is_taxonomy'] ) : ?>
				<?php
				...
				<?php if ( 'select' === $attribute_taxonomy->attribute_type ) : ?>
					<select multiple="multiple" style="width:100%" data-placeholder="<?php esc_attr_e( 'Select terms', 'sk' ); ?>" class="sk_attribute_values sk-select2" name="attribute_values[<?php echo $i; ?>][]">
						<?php
						$args = array(
							'orderby'    => 'name',
							'hide_empty' => 0,
					...
					<div class="sk-pre-defined-attribute-btn-group">
						<button class="sk-btn sk-btn-default plus sk-select-all-attributes"><?php esc_html_e( 'Select all', 'sk' ); ?></button>
						<button class="sk-btn sk-btn-default minus sk-select-no-attributes"><?php esc_html_e( 'Select none', 'sk' ); ?></button>

						<?php if ( sk_get_option( 'add_new_attribute', 'sk_selling', 'off' ) !== 'off' ) : ?>

						<button class="sk-btn sk-btn-default fr plus sk-add-new-attribute sk-right"><?php esc_html_e( 'Add new', 'sk' ); ?></button>

						<?php endif; ?>
					</div>
				<?php elseif ( 'text' === $attribute_taxonomy->attribute_type ) : ?>
					<?php // translators: %s:  WC DELIMITER ?>
                    <select name="attribute_values[<?php echo $i; ?>][]" id="" multiple style="width:100%" class="sk-select2" data-placeholder="<?php echo esc_attr( sprintf( __( 'Enter some text, or some attributes by "%s" separating values.', 'sk' ), WC_DELIMITER ) ); ?>" data-tags="true" data-allow-clear="true" data-token-separators="['|']">
                        <?php
			...
			<?php else : ?>
				<?php // translators: %s:  WC DELIMITER ?>
            	<select name="attribute_values[<?php echo $i; ?>][]" id="" multiple style="width:100%" class="sk-select2" data-placeholder="<?php echo esc_attr( sprintf( __( 'Enter some text, or some attributes by "%s" separating values.', 'sk' ), WC_DELIMITER ) ); ?>" data-tags="true" data-allow-clear="true" data-token-separators="['|']" data-values="[ 'Red', 'Green' ]">
