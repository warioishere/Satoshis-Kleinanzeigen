...
<div class="sk-product-variation-itmes">
    <h3 class="variation-topbar-heading">
        ...
        foreach ( $parent_data['attributes'] as $attribute ) {
            ...
            echo '<select class="sk-form-control" name="attribute_' . sanitize_title( $attribute['name'] ) . '[' . esc_attr( $loop ) . ']"><option value="">' . __( 'Any', 'sk-core' ) . ' ' . esc_html( wc_attribute_label( $attribute['name'] ) ) . '&hellip;</option>';
    ...
    <div class="actions">
        <i class="fas fa-bars sort tips" data-title="<?php esc_attr_e( 'Drag and drop, or click to set admin variation order', 'sk-core' ); ?>" aria-hidden="true" ></i>
        <i class="fas fa-sort-down fa-flip-horizointal toggle-variation-content" aria-hidden="true"></i>
        <a href="#" class="remove_variation delete" rel="<?php echo esc_attr( $variation_id ); ?>"><?php esc_html_e( 'Remove', 'sk-core' ); ?></a>
    </div>
        ...
        <div class="data">
            <div class="content-half-part thumbnail-checkbox-options">
                <div class="upload_image">
                    <a href="#" class="upload_image_button tips <?php echo ( $_thumbnail_id > 0 ) ? 'sk-img-remove' : ''; ?>" title="<?php ( $_thumbnail_id > 0 ) ? esc_attr_e( 'Remove this image', 'sk-core' ) : esc_attr_e( 'Upload an image', 'sk-core' ); ?>" rel="<?php echo esc_attr( $variation_id ); ?>">
                ...
                <div class="sk-form-group options">
                    <label><input type="checkbox" class="" name="variable_enabled[<?php echo esc_attr( $loop ); ?>]" <?php checked( $variation->post_status, 'publish' ); ?> /> <?php esc_html_e( 'Enabled', 'sk-core' ); ?></label>
                    <?php if ( 'sell_physical' !== sk_ext()->digital_product->get_selling_product_type() ) : ?>
                        ...
                        <label><input type="checkbox" class="variable_is_downloadable" name="variable_is_downloadable[<?php echo esc_attr( $loop ); ?>]" <?php checked( ! empty( $_downloadable ) ? $_downloadable : ( $sell_digital_products ? 'yes' : '' ), 'yes' ); ?> /> <?php esc_html_e( 'Downloadable', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" title="<?php esc_attr_e( 'Enable this option if access is given to a downloadable file upon purchase of a product', 'sk-core' ); ?>"></i></label>
                        <label><input type="checkbox" class="variable_is_virtual" name="variable_is_virtual[<?php echo esc_attr( $loop ); ?>]" <?php checked( ! empty( $_virtual ) ? $_virtual : ( $sell_digital_products ? 'yes' : '' ), 'yes' ); ?> /> <?php esc_html_e( 'Virtual', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" title="<?php esc_attr_e( 'Enable this option if a product is not shipped or there is no shipping cost', 'sk-core' ); ?>"></i></label>
                    <?php endif; ?>
                    <?php if ( 'yes' === get_option( 'woocommerce_manage_stock' ) ) : ?>
                        <label><input type="checkbox" class="variable_manage_stock" name="variable_manage_stock[<?php echo esc_attr( $loop ); ?>]" <?php checked( isset( $_manage_stock ) ? $_manage_stock : '', 'yes' ); ?> /> <?php esc_html_e( 'Manage stock?', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e( 'Enable this option to enable stock management at variation level', 'sk-core' ); ?>"></i></label>
                    <?php endif; ?>
            ...
            <div class="content-half-part">
                <?php if ( wc_product_sku_enabled() ) : ?>
                    <div class="sku">
                        <label><?php esc_html_e( 'SKU', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e( 'Enter a SKU for this variation or leave blank to use the parent product SKU.', 'sk-core' ); ?>"></i></label>
                ...
                <div class="stock-status hide_if_variation_manage_stock">
                    <label><?php esc_html_e( 'Stock status', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e( 'Controls whether or not the product is listed as "in stock" or "out of stock" on the frontend.', 'sk-core' ); ?>"></i></label>
            ...
            <div class="variable_pricing show_if_variable show_if_variable-subscription">
                ...
                <div class="content-half-part hide_if_variable-subscription" style="padding-right: 16px;">
                    ...
                    <input type="text" size="5" name="variable_regular_price[<?php echo esc_attr( $loop ); ?>]" value="<?php echo ( isset( $_regular_price ) ) ? esc_attr( $_regular_price ) : ''; ?>" class="sk-product-variable wc_input_price sk-form-control sk-product-regular-price-variable" placeholder="<?php esc_attr_e( 'Variation price (required)', 'sk-core' ); ?>" />
                </div>
                <div class="content-half-part content-width-subscription">
                    <label><?php echo __( 'Sale price', 'sk-core' ) . ' (' . get_woocommerce_currency_symbol() . ')'; ?> <a href="#" class="sale_schedule"><?php esc_html_e( 'Schedule', 'sk-core' ); ?></a><a href="#" class="cancel_sale_schedule" style="display:none"><?php esc_html_e( 'Cancel schedule', 'sk-core' ); ?></a></label>
            ...
            <?php if ( 'yes' === get_option( 'woocommerce_manage_stock' ) ) : ?>
                ...
                <div class="sk-form-group show_if_variation_manage_stock" style="display: none;">
                    <div class="content-half-part">
                        <label><?php esc_html_e( 'Stock quantity', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e( 'Enter a quantity to enable stock management at variation level, or leave blank to use the parent product\'s options.', 'sk-core' ); ?>"></i></label>
                    ...
                    <div class="sk-clearfix"></div>
                    <div class="content-half-part">
                        <label for="variable_low_stock_amount[<?php echo esc_attr( $loop ); ?>]"><?php esc_html_e( 'Low stock threshold', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e( 'When variation stock reaches this amount you will be notified by email.', 'sk-core' ); ?>"></i></label>
                ...
                <div class="weight-dimension">
                    <?php if ( wc_product_weight_enabled() ) : ?>
                        <div class="content-half-part hide_if_variation_virtual">
                            <label><?php echo __( 'Weight', 'sk-core' ) . ' (' . esc_html( get_option( 'woocommerce_weight_unit' ) ) . ')'; ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e( 'Enter a weight for this variation or leave blank to use the parent product weight.', 'sk-core' ); ?>"></i></label>
            ...
            <div>
                ...
                <div class="sk-form-group form-row-full">
                    ...
                    <select class="sk-form-control" name="variable_tax_class[<?php echo esc_attr( $loop ); ?>]">
                        <option value="parent" <?php selected( is_null( $_tax_class ), true ); ?>><?php esc_html_e( 'Same as parent', 'sk-core' ); ?></option>
                        <?php
            ...
            <div class="show_if_variation_downloadable" style="display: none;">
                <div class="sk-form-group downloadable_files">
                    ...
                    <table class="sk-table sk-table-striped">
                        <thead>
                            <div>
                                <th><?php esc_html_e( 'Name', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e( 'This is the name of the download shown to the customer.', 'sk-core' ); ?>"></i></th>
                                <th colspan="2"><?php esc_html_e( 'File URL', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e( 'This is the URL or absolute path to the file which customers will get access to. URLs entered here should already be encoded.', 'sk-core' ); ?>"></i></th>
                                <th>&nbsp;</th>
                            </div>
                        </thead>
                        <tbody>
            ...
            <div class="sk-form-group show_if_variation_downloadable" style="display: none;">
                <div class="content-half-part">
                    <label><?php esc_html_e( 'Download limit', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e( 'Leave blank for unlimited re-downloads.', 'sk-core' ); ?>"></i></label>
                    <input type="text" class="sk-form-control" name="variable_download_limit[<?php echo esc_attr( $loop ); ?>]" value="<?php echo ( isset( $_download_limit ) ) ? esc_attr( $_download_limit ) : ''; ?>" placeholder="<?php esc_attr_e( 'Unlimited', 'sk-core' ); ?>" />
                </div>
                <div class="content-half-part">
                    <label><?php esc_html_e( 'Download expiry', 'sk-core' ); ?> <i class="fas fa-question-circle tips" aria-hidden="true" data-title="<?php esc_attr_e( 'Enter the number of days before a download link expires, or leave blank.', 'sk-core' ); ?>"></i></label>
                    <input type="text" class="sk-form-control" name="variable_download_expiry[<?php echo esc_attr( $loop ); ?>]" value="<?php echo ( isset( $_download_expiry ) ) ? esc_attr( $_download_expiry ) : ''; ?>" placeholder="<?php esc_attr_e( 'Never', 'sk-core' ); ?>" />
                </div>
