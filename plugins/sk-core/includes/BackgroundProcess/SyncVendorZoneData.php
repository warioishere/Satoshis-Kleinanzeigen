...
class SyncVendorZoneData extends WC_Background_Process {
    ...
    public function __construct() {
        ...
        $this->action = 'sk_pro_sync_vendor_zone_data';
    ...
    public function dispatch() {
        ...
        if ( is_wp_error( $dispatched ) ) {
            sk_log(
                sprintf( 'Unable to dispatch SK Vendor Zone Data Sync: %s', $dispatched->get_error_message() ),
                'error'
            );
        }
    }
    ...
    protected function task( $args ) {
        ...
        if ( ! $zone instanceof \WC_Shipping_Zone ) {
            sk_log( sprintf( 'Invalid Vendor Zone Data Provided: %s', print_r( $args, true ) ) );
            return false;
        }
    ...
    protected function complete() {
        sk_log( 'Vendor Zone Data Sync completed', 'info' );
        parent::complete();
    }
}
