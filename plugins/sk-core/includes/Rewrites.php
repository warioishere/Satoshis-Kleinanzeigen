...
class Rewrites {
    ...
    public function __construct() {
        $this->custom_store_url = sk_get_option( 'custom_store_url', 'sk_general', 'store' );
    ...
    public function store_page_breadcrumb( $crumbs ) {
        ...
        $crumbs[1] = [ ucwords( $this->custom_store_url ), get_permalink( sk_get_option( 'store_listing', 'sk_pages' ) ) ];
        if ( ! $seller_info ) {
            $crumbs[2] = [ __( 'Error 404', 'sk-core' ), '' ];
            return $crumbs;
        }

        $crumbs[2] = [ $author, sk_get_store_url( $seller_info->data->ID ) ];
    ...
    public function register_rule() {
        ...
        do_action( 'sk_rewrite_rules_loaded', $this->custom_store_url );
    }
    ...
    public function resolve_wc_query_conflict( $query_vars ) {
        ...
        $dashboard_id = apply_filters( 'sk_get_dashboard_page_id', sk_get_option( 'dashboard', 'sk_pages', 0 ) );

        if ( ! empty( $post->ID ) && apply_filters( 'sk_get_current_page_id', $post->ID ) === absint( $dashboard_id ) ) {
    ...
    public function store_template( $template ) {
        ...
        if ( ! empty( $store_name ) ) {
            // Let biography and review templates handle their own URLs
            if ( get_query_var( 'biography' ) || get_query_var( 'store_review' ) ) {
                return $template;
            }
    ...
    public function product_edit_template( $template ) {
        ...
        $edit_product_url = sk_locate_template( 'products/edit-product-single.php' );

        return apply_filters( 'sk_get_product_edit_template', $edit_product_url );
    }
    ...
    public function remove_h1_from_heading_in_edit_product_page( $args ) {
        ...
        if ( ( sk_is_seller_dashboard() && isset( $wp->query_vars['settings'] ) && $wp->query_vars['settings'] === 'store' ) || ( ! empty( $_GET['product_id'] ) && ! empty( $_GET['action'] ) && ! empty( $_GET['_sk_edit_product_nonce'] ) ) ) {
    ...
    public function store_query_filter( $query ) {
        ...
        if ( ! is_admin() && $query->is_main_query() && ! empty( $author ) ) {
            ...
            $store_info  = sk_get_store_info( $seller_info->data->ID );
            $product_ppp = sk_get_option( 'store_products_per_page', 'sk_general', 12 );

            do_action( 'sk_store_page_query_filter', $query, $store_info );
            set_query_var( 'posts_per_page', apply_filters( 'sk_store_products_per_page', $product_ppp ) );
            ...
            $attributes = sk_get_chosen_taxonomy_attributes();
            if ( ! empty( $attributes ) ) {
            ...
            $query->set( 'tax_query', apply_filters( 'sk_store_tax_query', $tax_query ) );
    ...
    public function get_catalog_ordering_args( $orderby = '', $order = '' ) {
        ...
        if ( ! $orderby ) {
            ...
            if ( ! $orderby_value ) {
                if ( is_search() ) {
                    ...
                    $orderby_value = apply_filters( 'sk_default_catalog_orderby', get_option( 'woocommerce_default_catalog_orderby', 'menu_order' ) );
                }
            }
        ...
        return apply_filters( 'sk_get_store_products_ordering_args', $args, $orderby, $order );
    }
    ...
    public function maybe_flash_rewrite_rules() {
        $flash = get_option( 'sk_rewrite_rules_needs_flashing', 'yes' );

        if ( 'yes' !== $flash ) {
            return;
        }

        sk()->flush_rewrite_rules();

        update_option( 'sk_rewrite_rules_needs_flashing', 'no', true );
    }
}
