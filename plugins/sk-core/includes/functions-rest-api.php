<?php
if ( ! function_exists( 'sk_rest_validate_store_id' ) ) {
    /**
     ...
     */
    function sk_rest_validate_store_id( $value, $request, $key ) {
        ...
        $vendor = sk()->vendor->get( intval( $value ) );
        if ( $vendor->get_id() && $vendor->is_vendor() ) {
            return true;
        }

        // translators: 1) rest api endpoint key name
        return new WP_Error( 'rest_invalid_param', sprintf( esc_html__( 'No store found with given store id', 'sk-core' ), $key ), [ 'status' => 400 ] );
    }
}

if ( ! function_exists( 'sk_rest_validate_order_id' ) ) {
    /**
     ...
     * @return bool|WP_Error
     */
    function sk_rest_validate_order_id( $value, $request, $key ) {
        $attributes = $request->get_attributes();
