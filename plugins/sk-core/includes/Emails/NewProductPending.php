...
class NewProductPending extends WC_Email {
    ...
    public function __construct() {
        ...
        $this->title          = __( 'SK New Pending Product', 'sk-core' );
        $this->description    = __( 'New Pending Product emails are sent to chosen recipient(s) when a new product is created by vendors.', 'sk-core' );
        ...
        add_action( 'sk_new_product_added', array( $this, 'trigger' ), 30, 1 );
        add_action( 'sk_product_updated', array( $this, 'trigger' ), 30, 1 );
    ...
    public function get_default_subject() {
            return __( '[{site_title}] A New product is pending from ({store_name}) - {product_title}', 'sk-core' );
    }
    ...
    public function get_default_heading() {
            return __( 'New pending product added by Vendor {store_name}', 'sk-core' );
    }
    ...
    public function trigger( $product_id ) {
        ...
        // we've added _sk_new_product_email_sent from version 3.8.2
        // so, we are assuming if the meta doesn't exist, email was already sent to the client
        $email_sent = $product->get_meta( '_sk_new_product_email_sent' );
        ...
        $product->update_meta_data( '_sk_new_product_email_sent', 'yes' );
        $product->save();
        ...
        $this->placeholders['{price}']         = sk()->email->currency_symbol( $product->get_price() );
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            ...
            'recipient' => array(
                ...
                'description'   => sprintf( __( 'Enter recipients (comma separated) for this email. Defaults to %1$s.', 'sk-core' ), '<code>' . esc_attr( get_option( 'admin_email' ) ) . '</code>' ),
                'placeholder'   => '',
                'default'       => '',
                'desc_tip'      => true,
            ),
