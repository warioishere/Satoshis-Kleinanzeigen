...
namespace SK\Core\ProductCategory;
...
class Helper {
    ...
    public static function product_category_selection_is_single() {
        return 'single' === sk_get_option( 'product_category_style', 'sk_selling', 'single' );
    }
    ...
    public static function get_ancestors_html( $term ) {
        ...
        foreach ( $all_parents as $index => $value ) {
            ...
            $label = '<span class="sk-selected-category-product">' . $name . '</span><span class="sk-selected-category-icon"><i class="fas fa-chevron-right"></i></span>';
            $html .= $label;
        }

        $name = get_term_field( 'name', $term, 'product_cat' );
        $name = is_wp_error( $name ) ? '' : $name;
        $html .= '<span class="sk-selected-category-product sk-cat-selected">' . $name . '</span>';

        return $html;
    }

    /**
     * Enqueue styles and scripts and localize for sk multi-step category.
     *
     *
     * @return void
     */
    public static function enqueue_and_localize_sk_multistep_category() {
        wp_enqueue_style( 'sk-product-category-ui-css' );
        ...
        wp_localize_script( 'product-category-ui', 'sk_product_category_data', $data );
    }
     ...
     * in the database. it will not consider if the setting in sk setting is single or multiple category selection.
    ...
    public static function get_product_categories_tree(): array {
         ...
         */
        return apply_filters( 'sk_get_product_categories_tree', $tree, $categories );
    }
}
