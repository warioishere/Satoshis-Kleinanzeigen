...
class Review extends DashboardModule {
    ...
    protected function register_extras(): void {
        ...
        add_action( 'sk_review_content_status_filter', array( $this, 'sk_review_status_filter' ), 10, 2 );
        ...
        add_action( 'wp_ajax_sk_update_comment', array( $this, 'ajax_update_comment' ) );
    ...
    public function render_dashboard( $query_vars ) {
        if ( isset( $query_vars['reviews'] ) ) {
            if ( ! current_user_can( 'sk_view_review_menu' ) ) {
                ...
                sk_get_template_part( 'review/reviews', '', array( 'pro' => true ) );
                return;
            }
        }
    }
     ...
     * @return void
     */
    public function sk_review_header_render() {
        sk_get_template_part( 'review/header', '', array( 'pro' => true ) );
    }

    /**
     * Render sk review content
     *
     *
     * @return void
     */
    public function sk_review_content_render() {
        $this->reviews_view();
    }
    ...
    public function get_count( $post_type ) {
        ...
        $counts = sk_count_comments( $post_type, sk_get_current_user_id() );
    ...
    /**
     * SK Render Review Status filter
     *
     *
     * @param  string $post_type
     * @param  object $counts
     ...
     * @return void
     */
    public function sk_review_status_filter( $post_type, $counts ) {
    ...
    public function review_content_listing( $post_type ) {
        if ( ! current_user_can( 'sk_manage_reviews' ) && ! current_user_can( 'sk_view_reviews' ) ) {
            return;
        }

        $comment_status = 'all';
        if ( isset( $_POST['sk_comment_nonce'] ) && wp_verify_nonce( sanitize_key( wp_unslash( $_POST['sk_comment_nonce'] ) ), 'sk_comment_nonce_action' ) ) {
    ...
    public function pagination( $post_type ) {
        ...
        $base_url = sk_get_navigation_url( 'reviews' );
     ...
     * @return string
     */
    public function sk_render_listing_table_body( $post_type ) {
        $status    = $this->page_status();
        $limit     = $this->limit;
    ...
    public function handle_status() {
        ...
        if ( ! wp_verify_nonce( $_POST['sk_comment_nonce'], 'sk_comment_nonce_action' ) || ! is_user_logged_in() ) {
            return;
        }
        ...
        $redirect_to = add_query_arg( array( 'comment_status' => $current_status ), sk_get_navigation_url( 'reviews' ) );
    ...
    public function count_status( $post_type, $status ) {
        ...
        return $totalcomments = $wpdb->get_var(
            ...
            FROM $wpdb->comments, $wpdb->posts
            WHERE $wpdb->posts.post_author=sk_get_current_user_id() AND
            $wpdb->posts.post_status='publish' AND
    ...
    public function render_store_tab_comment_list( $comments, $store_id ) {
                ...
                if ( $single_comment->comment_approved ) {
                    ...
                    <li <?php comment_class(); ?> itemtype="http://schema.org/Review" itemscope="" itemprop="reviews">
                        <div class="review_comment_container">
                            <div class="sk-review-author-img"><?php echo $comment_author_img; ?></div>
        ...
        return apply_filters( 'sk_seller_tab_reviews_list', $review_list, $store_id );
    }
