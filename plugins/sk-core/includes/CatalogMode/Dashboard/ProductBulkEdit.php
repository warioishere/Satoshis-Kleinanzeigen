...
class ProductBulkEdit {
    ...
    public function __construct() {
        ...
        add_filter( 'sk_bulk_product_statuses', [ $this, 'bulk_product_catalog_options' ], 22, 1 );
        add_action( 'sk_bulk_product_status_change', [ $this, 'save_bulk_edit_catalog_mode_data' ], 10, 2 );
        add_action( 'sk_product_dashboard_errors', [ $this, 'display_product_update_message' ], 10, 1 );
    }
    ...
    public function bulk_product_catalog_options( $bulk_statuses ) {
        ...
        return sk_array_insert_after(
            $bulk_statuses,
            [
                'enable_catalog_mode'  => __( 'Enable Catalog Mode', 'sk-core' ),
                'disable_catalog_mode' => __( 'Disable Catalog Mode', 'sk-core' ),
            ],
    ...
    public function save_bulk_edit_catalog_mode_data( $status, $product_ids ) {
        ...
        if ( ! empty( $product_ids ) ) {
            foreach ( $product_ids as $product_id ) {
                ...
                update_post_meta( $product_id, '_sk_catalog_mode', $catalog_mode_data );
            }
        }
        wp_safe_redirect(
            add_query_arg(
    ...
    public function display_product_update_message( $type ) {
        ...
        if ( $count > 0 ) {
            sk_get_template_part(
                'global/sk-success', '', [
                    ...
                    // translators: %d is product count.
                        _n( '%s product has been successfully updated.', '%s products have been successfully updated.', $count, 'sk-core' ),
            ...
            sk_get_template_part(
                'global/sk-error', '', [
                    'deleted' => true,
                    'message' => __( 'No product data has been updated.', 'sk-core' ),
                ]
            );
        }
    }
