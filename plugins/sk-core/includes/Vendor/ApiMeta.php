 ...
 * ApiMeta Class.
 *
 * Handles SK vendor user meta registration for the REST API.
 */
class ApiMeta {
	/**
	 * Constructor.
	...
	/**
	 * Registers SK specific user data to the WordPress user API.
	 *
	 *
	 * @return void
	 */
	...
	public function get_user_data_values( $user ) {
		 ...
		 * @param array $user Current user data from REST API.
		 */
		return apply_filters( 'sk_vendor_api_meta_get_user_data_values', $values, $user );
	}

	/**
	 * We store some SK specific user meta attached to users endpoint,
	 * so that we can track certain preferences or values for vendors.
	 * Additional fields can be added in the function below, and then used via SK's currentUser data.
	 *
	 *
	...
	public function get_user_data_fields() {
		 ...
		 */
		return apply_filters( 'sk_vendor_get_user_data_fields', [] );
	}
