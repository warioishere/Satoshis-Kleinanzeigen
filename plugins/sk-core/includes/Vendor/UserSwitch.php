...
class UserSwitch {
    ...
    public function __construct() {
        add_filter( 'sk_admin_localize_script', [ $this, 'add_localize_data' ], 15 );
        add_filter( 'sk_admin_dashboard_vendors_settings', [ $this, 'add_localize_data' ] );
        add_filter( 'sk_rest_store_additional_fields', [ $this, 'populate_switch_url' ], 2, 3 );
        add_action( 'sk_dashboard_content_inside_before', [ $this, 'show_user_switching_message' ], 9 );
    }
    ...
    public function add_localize_data( $localize_data ) {
        ...
        $localize_data['is_vendor_legacy_page']       = get_transient( 'sk_legacy_vendors_page' );
        ...
        if ( $old_user ) {
            ...
            <div class="sk-alert sk-alert-info">
                <span id="user_switching" class="user-switching-message">
                    ...
                    ?>
                    <span class="fa fa-user" style="color: var(--sk-button-background-color, #7047EB)" aria-hidden="true"></span>
                    <?php
                        $message       = '';
                        ...
                        if ( $just_switched ) {
                            $message = esc_html( sprintf(
                                ...
                                __( 'Switched to %1$s (%2$s).', 'sk-core' ),
                                $user->display_name,
                                $user->user_login
                            ) );
                        }
                        ...
                        $message .= sprintf(
                            ...
                            esc_html( sprintf(
                                /* Translators: 1: user display name; 2: username; */
                                __( 'Switch back to %1$s (%2$s)', 'sk-core' ),
                                $old_user->display_name,
                                $old_user->user_login
                            ) )
                        );

                        $message = apply_filters( 'sk_user_switching_switched_message', $message, $user, $old_user, $switch_back_url, $just_switched );
                    ...
                    <?php
                        if ( isset( $_GET['switched_back'] ) ) {
                            echo esc_html( sprintf(
                                /* Translators: 1: user display name; 2: username; */
                                __( 'Switched back to %1$s (%2$s).', 'sk-core' ),
                                $user->display_name,
                                $user->user_login
                            ) );
                        } else {
