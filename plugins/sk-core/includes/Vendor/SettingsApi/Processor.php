...
/**
 * SK Vendor Settings API Processor.
 *
 */
class Processor {
    ...
    public function get_settings_page_list() {
        return array_map(
            ...
            apply_filters( 'sk_vendor_rest_settings_page_list', [] )
        );
    }
    ...
    public function get_settings_group( string $group_id ) {
        ...
        $settings = apply_filters( 'sk_vendor_rest_settings_for' . $group_id . '_page', [] );
    ...
    public function get_single_settings_field( string $group_id, string $parent_id, string $id ) {
        ...
        $error = new WP_Error( 'sk_rest_setting_option_not_found', __( 'Setting Option not found', 'sk-core' ), [ 'status' => 404 ] );
    ...
    public function populate_settings_elements( $settings ) {
        ...
        return apply_filters( 'sk_vendor_rest_settings_element_value_population', $this->populate_single_settings_links_value( $settings ), $settings_values, $settings['parent_id'] );
    }
    ...
    public function format_settings_elements_for_saving( string $settings_group_id, array $settings_values ) {
        ...
        foreach ( $settings_values as $settings ) {
            ...
            if ( is_wp_error( $settings_element ) ) {
                throw new Exception( __( 'Settings Element not found', 'sk-core' ) );
            }
            ...
            if ( 'section' === $settings_element['type']  ) {
                foreach ( $fields_values as $field_id => $value ) {
                    ...
                    if ( is_wp_error( $settings_element_fields ) ) {
                        throw new Exception( __( 'Settings Element Fields not found', 'sk-core' ) );
                    }
    ...
    public function save_settings_group( array $settings, string $group_id ) {
        ...
        $settings_updated = sk()->vendor->update( $this->vendor->get_id(), $settings_data_array );
        ...
        do_action( 'sk_rest_' . $group_id . '_settings_after_update', $this->vendor, $settings );
    ...
    protected function search_group( string $group_id ) {
        ...
        if ( false === $found ) {
            return new WP_Error( 'sk_rest_setting_group_not_found', __( 'Setting group not found', 'sk-core' ), [ 'status' => 404 ] );
        }
    ...
    protected function search_single_settings( string $group_id, string $id ) {
        ...
        if ( false === $single_settings_array_position ) {
            return new WP_Error( 'sk_rest_setting_option_not_found', __( 'Setting Option not found', 'sk-core' ), [ 'status' => 404 ] );
        }
