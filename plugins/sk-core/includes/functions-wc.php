...
add_filter( 'woocommerce_cart_shipping_packages', 'sk_custom_split_shipping_packages' );
 ...
 * @return array
 */
function sk_custom_split_shipping_packages( $packages ) {
    ...
    foreach ( $cart_content as $key => $item ) {
        ...
        if ( \SK\Core\Shipping\Methods\ProductShipping::is_product_disable_shipping( $item['product_id'] ) ) {
            continue;
        }
    ...
    return apply_filters( 'sk_cart_shipping_packages', $packages );
}

add_filter( 'woocommerce_shipping_package_name', 'sk_change_shipping_pack_name', 10, 3 );
 ...
 * @return string
 */
function sk_change_shipping_pack_name( $title, $i, $package ) {
    ...
    $shipping_label = sprintf( '%s %s', __( 'Shipping: ', 'sk' ), ! empty( $store_info['store_name'] ) ? $store_info['store_name'] : '' );

    return apply_filters( 'sk_shipping_package_name', $shipping_label, $i, $package );
}

add_action( 'woocommerce_checkout_create_order_shipping_item', 'sk_add_shipping_pack_meta', 10, 4 );
 ...
 * @return void
 */
function sk_add_shipping_pack_meta( $item, $package_key, $package, $order ) {
