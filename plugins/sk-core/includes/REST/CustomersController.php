...
class CustomersController extends WC_REST_Customers_Controller {
        ...
        register_rest_route(
            $this->namespace, '/' . $this->rest_base . '/search', array(
				array(
					...
					'args'                => array(
						'search' => array(
							'description' => __( 'Search string.', 'sk-core' ),
							'type'        => 'string',
							'required'    => true,
						),
						'exclude' => array(
							'description' => __( 'Comma-separated list of customer IDs to exclude.', 'sk-core' ),
							'type'        => 'string',
						),
					),
				),
    ...
    protected function check_permission( $request, $action ) {
        if ( ! $this->check_vendor_permission() ) {
            $messages = [
                'view'   => __( 'Sorry, you cannot list resources.', 'sk-core' ),
                'create' => __( 'Sorry, you are not allowed to create resources.', 'sk-core' ),
                'edit'   => __( 'Sorry, you are not allowed to edit this resource.', 'sk-core' ),
                'delete' => __( 'Sorry, you are not allowed to delete this resource.', 'sk-core' ),
                'batch'  => __( 'Sorry, you are not allowed to batch update resources.', 'sk-core' ),
                'search' => __( 'Sorry, you are not allowed to search customers.', 'sk-core' ),
            ];
            return new WP_Error( "sk_rest_cannot_$action", $messages[ $action ], [ 'status' => rest_authorization_required_code() ] );
        }
        return true;
    }
    ...
    public function check_vendor_permission(): bool {
        return sk_is_user_seller( sk_get_current_user_id() );
    }
    ...
    public function search_customers( $request ) {
        if ( ! current_user_can( 'edit_shop_orders' ) ) {
            return new WP_Error( 'sk_rest_cannot_search', __( 'You do not have permission to search customers.', 'sk-core' ), [ 'status' => rest_authorization_required_code() ] );
        }
        ...
        if ( empty( $term ) ) {
            return new WP_Error( 'sk_rest_empty_search', __( 'Search term is required.', 'sk-core' ), [ 'status' => 400 ] );
        }
        ...
        foreach ( $ids as $id ) {
            if ( ! sk_customer_has_order_from_this_seller( $id ) ) {
                continue;
            }
        ...
        /**
         * Filter the found customers for SK REST API search.
         *
         ...
         */
        $found_customers = apply_filters( 'sk_json_search_found_customers', $found_customers, $term, $exclude, $limit );
    ...
    protected function prepare_object_for_database( $request, $creating = false ) {
        ...
        if ( ! $customer instanceof WC_Customer ) {
            return new WP_Error( 'sk_rest_invalid_customer', __( 'Invalid customer.', 'sk-core' ), [ 'status' => 400 ] );
        }

        // Add any SK-specific customer preparation here

        return apply_filters( "sk_rest_pre_insert_{$this->post_type}_object", $customer, $request, $creating );
    }
