...
*
*/
class AdminReportController extends SkRESTAdminController {
    ...
    public function get_summary( $request ) {
        ...
        $sales = sk_get_sales_count( $from, $to, $seller_id );

        $data = array(
            'products' => sk_get_product_count( $from, $to, $seller_id ),
            'vendors'  => sk_get_seller_count( $from, $to ),
            'sales'    => $sales['sales'],
            'orders'   => $sales['orders'],
    ...
    public function get_overview( $request ) {
        ...
        $data          = sk_admin_report_data( $group_by, '', $start_date->format( 'Y-m-d' ), $end_date->format( 'Y-m-d' ), $seller_id );
        $data          = apply_filters( 'sk_get_overview_data', $data, $group_by, $start_date->format( 'Y-m-d' ), $end_date->format( 'Y-m-d' ), $seller_id );
        ...
        foreach ( $data as $row ) {
            if ( 'month' === $group_by ) {
                ...
                $date = sk_current_datetime()->modify( $row->order_date )->format( 'Y-m-d' );
            }
        ...
        $response = array(
            ...
            'datasets' => array(
                array(
                    'label'           => __( 'Total Sales', 'sk-core' ),
                    'borderColor'     => '#3498db',
                    'fill'            => false,
                    'data'            => array_values( $order_amounts ),
                    'tooltipLabel'    => __( 'Total', 'sk-core' ),
                    'tooltipPrefix'   => html_entity_decode( get_woocommerce_currency_symbol() ),
                ),
                array(
                    'label'           => __( 'Number of orders', 'sk-core' ),
                    'borderColor'     => '#1abc9c',
                    'fill'            => false,
                    'data'            => array_values( $order_counts ),
                    'tooltipLabel'    => __( 'Orders', 'sk-core' ),
                ),
                array(
                    'label'           => __( 'Commision', 'sk-core' ),
                    'borderColor'     => '#73a724',
                    'fill'            => false,
