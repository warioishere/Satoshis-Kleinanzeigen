...
class ProductAttributeTermsController extends WC_REST_Product_Attribute_Terms_V1_Controller {
    ...
    public function delete_item( $request ) {
        ...
        if ( empty( $term ) ) {
            return new WP_Error( 'sk_rest_no_term', __( 'The term cannot found', 'sk-core' ), array( 'status' => 500 ) );
        }
        ...
        if ( ! $retval ) {
            return new WP_Error( 'sk_rest_cannot_delete', __( 'The resource cannot be deleted.', 'sk-core' ), array( 'status' => 500 ) );
        }

        do_action( "sk_rest_delete_{$taxonomy}", $term, $response, $request );
