...
use SK\Core\Admin\OnboardingSetup\AdminSetupGuide;
use WP_REST_Request;
use WP_REST_Response;
use WP_REST_Server;

class AdminSetupGuideController extends SkBaseAdminController {
    ...
    public function get_items( $request ): WP_REST_Response {
         ...
         */
        $setup_guide = sk_get_container()->get( AdminSetupGuide::class );
         ...
         */
        $steps = apply_filters( 'sk_admin_setup_guide_steps_progress', $steps );
    ...
    public function get_item( $request ) {
        ...
        if ( false === $step_index ) {
            return new \WP_Error( 'sk_rest_invalid_step', esc_html__( 'Invalid step.', 'sk-core' ), [ 'status' => 404 ] );
        }
         ...
         */
        $step_array = apply_filters( 'sk_admin_setup_guide_step_response', $step->populate(), $step );
    ...
    public function update_item( $request ) {
        ...
        try {
            ...
            return new \WP_Error( 'sk_rest_invalid_step', $e->getMessage(), [ 'status' => 400 ] );
        }
