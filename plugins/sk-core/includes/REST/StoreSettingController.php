...
class StoreSettingController extends StoreController {
                ...
                [
                    ...
                    'args'                => [
                        'vendor_id' => [
                            ...
                            'description'       => __( 'Optional vendor ID', 'sk-core' ),
                        ],
                    ],
                ],
                [
    ...
    public function update_settings( $request ) {
        ...
        $store = sk()->vendor->get( $vendor_id );

        do_action( 'sk_rest_store_settings_after_update', $store, $request );
    ...
    public function get_settings_permission_callback() {
        ...
        if ( empty( $vendor->get_id() ) ) {
            return new WP_Error( 'no_store_found', __( 'No vendor found', 'sk-core' ), [ 'status' => 404 ] );
        }
    ...
    protected function get_vendor( $request = null ) {
        ...
        if ( $vendor_id ) {
            $vendor = sk()->vendor->get( (int) $vendor_id );
        } else {
            $current_user = sk_get_current_user_id();

            if ( ! $current_user ) {
                return new WP_Error( 'Unauthorized', __( 'You are not logged in', 'sk-core' ), [ 'code' => 401 ] );
            }

            if ( $current_user ) {
                $vendor = sk()->vendor->get( $current_user );
            }
        }
    ...
    public function prepare_item_for_response( $store, $request, $additional_fields = [] ) {
        ...
        $data     = array_merge( $data, apply_filters( 'sk_rest_store_settings_additional_fields', $additional_fields, $store, $request ) );
        $response = rest_ensure_response( $data );
        $response->add_links( $this->prepare_links( $data, $request ) );

        return apply_filters( 'sk_rest_prepare_store_settings_item_for_response', $response );
    }
}
