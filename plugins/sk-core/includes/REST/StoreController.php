...
class StoreController extends WP_REST_Controller {
        ...
        register_rest_route(
            $this->namespace, '/' . $this->base, [
                [
                    ...
                    'args'                => apply_filters( 'sk_rest_api_store_collection_params', $this->get_store_collection_params() ),
            ...
            $this->namespace, '/' . $this->base . '/(?P<id>[\d]+)', [
                ...
                [
                    ...
                    'args'                => [
                        'reassign' => [
                            'type'        => 'integer',
                            'description' => __( 'Reassign the deleted user\'s posts and links to this user ID.', 'sk-core' ),
                            'required'    => true,
                        ],
                    ],
                ],
            ...
            $this->namespace, '/' . $this->base . '/(?P<id>[\d]+)/contact', [
                ...
                [
                    ...
                    'args'                => [
                        'name'    => [
                            'type'        => 'string',
                            'description' => __( 'Your Name', 'sk-core' ),
                            'required'    => true,
                        ],
                        'email'   => [
                            'type'        => 'string',
                            'format'      => 'email',
                            'description' => __( 'Your email', 'sk-core' ),
                            'required'    => true,
                        ],
                        'message' => [
                            'type'        => 'string',
                            'description' => __( 'Your Message', 'sk-core' ),
                            'required'    => true,
                        ],
                    ],
                    'permission_callback' => '__return_true',
        ...
        register_rest_route(
            $this->namespace, '/' . $this->base . '/(?P<id>[\d]+)/categories', [
                'args' => [
                    ...
                    'best_selling' => [
                        'description' => __( 'Get Best Selling Products Category.', 'sk-core' ),
                        'type'        => 'boolean',
                        'default'     => false,
                        'required'    => false,
                    ],
    ...
    public function get_stores( $request ) {
        ...
        $args = apply_filters( 'sk_seller_listing_args', $args, $request );
        ...
        $args = apply_filters( 'sk_rest_get_stores_args', $args, $request );

        $stores = sk()->vendor->get_vendors( $args );
        ...
        $response = $this->format_collection_response( $response, $request, sk()->vendor->get_total() );
    ...
    public function get_store( $request ) {
		...
		$store = sk()->vendor->get( $store_id );

        if ( ! $store || ! $store->get_id() ) {
            return new WP_Error(
                'sk_rest_store_not_found',
                __( 'Store not found.', 'sk-core' ),
                [ 'status' => 404 ]
            );
        }
    ...
    public function delete_store( $request ) {
        ...
        if ( ! empty( $reassign ) ) {
            if ( $reassign === $store_id || ! get_userdata( $reassign ) ) {
                return new WP_Error( 'rest_user_invalid_reassign', __( 'Invalid user ID for reassignment.', 'sk-core' ), [ 'status' => 400 ] );
            }
        }

        $vendor   = sk()->vendor->delete( $store_id, $reassign );
    ...
    public function update_store( $request ) {
        ...
        if ( ! $this->can_access_vendor_store( $store->get_id() ) ) {
            return new WP_Error(
                'sk_rest_store_cannot_access',
                __( 'You do not have permission to access this store.', 'sk-core' ),
                [ 'status' => 403 ]
            );
        }
        ...
        $updated_store_id = sk()->vendor->update( $store->get_id(), $params );

        if ( is_wp_error( $updated_store_id ) ) {
            return new WP_Error( $updated_store_id->get_error_code(), $updated_store_id->get_error_message() );
        }

        $store = sk()->vendor->get( $updated_store_id );

        do_action( 'sk_rest_stores_update_store', $store, $request );
     ...
     *
     *
     * @param \SK\Core\Vendor\Vendor $store Store object.
    ...
    protected function get_restricted_fields_for_update( $store, $request ) {
        ...
        $is_vendor = sk_is_user_seller( get_current_user_id(), true );
        $restricted_fields = [];
        ...
        return apply_filters( 'sk_rest_store_restricted_fields_for_update', $restricted_fields, $store, $request );
    }
    ...
    public function create_store( $request ) {
        ...
        $store = sk()->vendor->create( $params );

        if ( is_wp_error( $store ) ) {
            return new WP_Error( $store->get_error_code(), $store->get_error_message() );
        }

        do_action( 'sk_rest_stores_create_store', $store, $request );
    ...
    public function format_collection_response( $response, $request, $total_items ) {
        ...
        if ( function_exists( 'sk_get_seller_status_count' ) && current_user_can( 'manage_woocommerce' ) ) {
            $counts = sk_get_seller_status_count();
    ...
    public function get_store_reviews( $request ) {
        ...
        if ( empty( $store_id ) ) {
            return new WP_Error( 'no_store_found', __( 'No store found', 'sk-core' ), [ 'status' => 404 ] );
        }
        ...
        if ( sk()->is_pro_exists() ) {
            if ( sk_ext()->module->is_active( 'store_reviews' ) ) {
                ...
                if ( empty( $query->posts ) ) {
                    return new WP_Error( 'no_reviews_found', __( 'No reviews found', 'sk-core' ), [ 'status' => 404 ] );
                }
                ...
                $status                 = '1';
                $comments               = $sk_template_reviews->comment_query( $store_id, $post_type, $limit, $status, $paged );
    ...
    public function prepare_item_for_response( $store, $request, $additional_fields = [] ) {
        ...
        $data     = array_merge( $data, apply_filters( 'sk_rest_store_additional_fields', $additional_fields, $store, $request, $is_authorized ) );
        $response = rest_ensure_response( $data );
        $response->add_links( $this->prepare_links( $data, $request ) );

        return apply_filters( 'sk_rest_prepare_store_item_for_response', $response );
    }
    ...
    protected function get_restricted_fields_for_view( $store, $request ) {
        ...
        if ( ! $is_authorized ) {
            ...
            if ( sk_is_vendor_info_hidden( 'email' ) || ! $store->show_email() ) {
        ...
        return apply_filters( 'sk_rest_store_restricted_fields_for_view', $restricted_fields, $store, $request );
    }
    ...
    public function prepare_reviews_for_response( $item, $request, $additional_fields = [] ) {
        if ( sk()->is_pro_exists() && sk_ext()->module->is_active( 'store_reviews' ) ) {
        ...
        if ( ! empty( $params['email'] ) ) {
            ...
            if ( ! is_email( $user_email ) ) {
                $response = [
                    'email'     => $user_email,
                    'available' => false,
                    'message'   => __( 'This email address is not valid', 'sk-core' ),
                ];
    ...
    public function send_email( $request ) {
        $params = $request->get_params();
        $vendor = sk()->vendor->get( $params['id'] );

        if ( ! $vendor instanceof Vendor || ! $vendor->get_id() ) {
            return rest_ensure_response(
                new WP_Error(
                    'vendor_not_found',
                    __( 'No vendor is found to be send an email.', 'sk-core' ),
                    [
                        'status' => 404,
                    ]
                )
         ...
         *
         */
        do_action( "sk_rest_pre_{$this->base}_send_email", $request, true );
        ...
        WC()->mailer()->emails['SK_Email_Contact_Seller']->trigger( $vendor_email, $sender_name, $sender_email, $sender_message );

        $response = apply_filters(
            "sk_{$this->base}_send_email_response", [
                'store_id'       => $vendor->get_id(),
                'data'           => __( 'Email sent successfully!', 'sk-core' ),
    ...
    public function update_vendor_status( $request ) {
        if ( ! in_array( $request['status'], [ 'active', 'inactive' ], true ) ) {
            return new WP_Error( 'no_valid_status', __( 'Status parameter must be active or inactive', 'sk-core' ), [ 'status' => 400 ] );
        }

        $store_id = ! empty( $request['id'] ) ? $request['id'] : 0;

        if ( empty( $store_id ) ) {
            return new WP_Error( 'no_vendor_found', __( 'No vendor found for updating status', 'sk-core' ), [ 'status' => 400 ] );
        }
    ...
    public function batch_update( $request ) {
        ...
        if ( empty( $params ) ) {
            return new WP_Error( 'no_item_found', __( 'No items found for bulk updating', 'sk-core' ), [ 'status' => 404 ] );
        }
    ...
    public function get_store_collection_params(): array {
        $params = array(
            'status'     => array(
                'description'       => __( 'Status of the store', 'sk-core' ),
                'type'              => 'string',
                'default'           => 'all',
            ...
            'orderby'     => array(
                'description'       => __( 'Store List Order By', 'sk-core' ),
                'type'              => 'string',
                'sanitize_callback' => 'sanitize_text_field',
                'validate_callback' => 'rest_validate_request_arg',
            ),
            'order'     => array(
                'description'       => __( 'Store List Order', 'sk-core' ),
                'type'              => 'string',
                'sanitize_callback' => 'sanitize_text_field',
                'validate_callback' => 'rest_validate_request_arg',
            ),
    ...
    public function get_store_update_params(): array {
        $params = [
            'id'                        => [
                'description'       => esc_html__( 'Unique identifier for the vendor.', 'sk-core' ),
                'type'              => 'integer',
                'required'          => true,
                'sanitize_callback' => 'absint',
            ],
            'first_name'                => [
                'description'       => esc_html__( 'Store owner first name.', 'sk-core' ),
                'type'              => 'string',
                'sanitize_callback' => 'sanitize_text_field',
            ],
            'last_name'                 => [
                'description'       => esc_html__( 'Store owner last name.', 'sk-core' ),
                'type'              => 'string',
                'sanitize_callback' => 'sanitize_text_field',
            ],
            'user_nicename'             => [
                'description'       => esc_html__( 'User nice name.', 'sk-core' ),
                'type'              => 'string',
                'sanitize_callback' => 'sanitize_text_field',
            ],
            'user_pass'                 => [
                'description' => esc_html__( 'User password.', 'sk-core' ),
                'type'        => 'string',
            ],
            'email'                     => [
                'description'       => esc_html__( 'Store email address.', 'sk-core' ),
                'type'              => 'string',
                'format'            => 'email',
                'validate_callback' => 'sanitize_email',
            ],
            'store_name'                => [
                'description'       => esc_html__( 'Store name.', 'sk-core' ),
                'type'              => 'string',
                'sanitize_callback' => 'sanitize_text_field',
            ],
            'phone'                     => [
                'description'       => esc_html__( 'Store phone number.', 'sk-core' ),
                'type'              => 'string',
                'sanitize_callback' => 'sanitize_text_field',
            ],
            'show_email'                => [
                'description' => esc_html__( 'Show email in store.', 'sk-core' ),
                'type'        => 'boolean',
                'default'     => false,
            ],
            'gravatar_id'               => [
                'description' => esc_html__( 'Gravatar ID.', 'sk-core' ),
                'type'        => 'integer',
            ],
            'banner_id'                 => [
            ...
            'social'                    => [
                'description' => esc_html__( 'Social profiles.', 'sk-core' ),
                'type'        => 'object',
                'properties'  => [
                    'fb'        => [ 'type' => 'string' ],
                    'twitter'   => [ 'type' => 'string' ],
            ...
            'address'                   => [
                'description' => esc_html__( 'Store address.', 'sk-core' ),
                'type'        => 'object',
                'properties'  => [
            ...
            // Admin only fields
            'enabled'                   => [
                'description' => esc_html__( 'Enable selling.', 'sk-core' ),
                'type'        => 'boolean',
            ],
            'featured'                  => [
                'description' => esc_html__( 'Featured vendor.', 'sk-core' ),
                'type'        => 'boolean',
            ],
            'trusted'                   => [
                'description' => esc_html__( 'Trusted vendor.', 'sk-core' ),
                'type'        => 'boolean',
            ],
        ];

        return apply_filters( 'sk_rest_api_store_update_params', $params );
    }
}
