 ...
 * 
 */
class ProductController extends SkRESTController {
    ...
    public function __construct() {
        add_filter( "sk_rest_{$this->post_type}_object_query", [ $this, 'add_only_downloadable_query' ], 10, 2 );
    }
    ...
    public function add_only_downloadable_query( $args, $request ) {
        if ( true === sk_string_to_bool( $request->get_param( 'only_downloadable' ) ) ) {
            $args['meta_query'][] = [
                'key'     => '_downloadable',
                'value'   => 'yes',
                'compare' => '=',
    ...
    public function get_product_collection_params() {
        ...
        $schema['only_downloadable'] = [
            'description' => __( 'If truthy value then only downloadable products will be returned', 'sk-core' ),
            'type' => [ 'boolean', 'string' ],
            'enum' => [ true, false, 0, 1 ],
            'sanitize_callback' => 'sk_string_to_bool',
            'validate_callback' => 'sk_string_to_bool',
            'default' => false,
        ];
                ...
                [
                    ...
                    'args'                => [
                        'force' => [
                            'type'        => 'boolean',
                            'default'     => false,
                            'description' => __( 'Whether to bypass trash and force deletion.', 'sk-core' ),
                        ],
                    ],
                ],
            ]
        ...
        register_rest_route(
            $this->namespace, '/' . $this->base . '/(?P<id>[\d]+)/related', [
                'args' => [
                    ...
                    'per_page' => [
                        'description' => __( 'Number of product you want to get top rated product', 'sk-core' ),
                        'type'        => 'integer',
                        'default'     => 10,
                    ],
                ],
        ...
        register_rest_route(
            $this->namespace, '/' . $this->base . '/top_rated', [
                'args' => [
                    ...
                    'page'      => [
                        'description' => __( 'Number of page number', 'sk-core' ),
                        'type'        => 'integer',
                        'default'     => 1,
                    ],
                    'seller_id' => [
                        'description' => __( 'Top rated product for specific vendor', 'sk-core' ),
                        'type'        => 'integer',
                    ],
                ],
                [
    ...
    public function validation_before_create_item( $request ) {
        $store_id = $this->validate_post_author_override( $request, sk_get_current_user_id() );

        if ( empty( $store_id ) ) {
            return new WP_Error( 'no_store_found', __( 'No seller found', 'sk-core' ), [ 'status' => 404 ] );
        }

        if ( ! sk_is_seller_enabled( $store_id ) && ! current_user_can( 'manage_options' ) ) {
            return new WP_Error( 'invalid_request', __( 'Error! Your account is not enabled for selling, please contact the admin', 'sk-core' ), [ 'status' => 400 ] );
        }

        if ( ! empty( $request['id'] ) ) {
            // translators: 1) %s: post type name
            return new WP_Error( "woocommerce_rest_{$this->post_type}_exists", sprintf( __( 'Cannot create existing %s.', 'sk-core' ), 'product' ), [ 'status' => 400 ] );
        }

        if ( empty( $request['name'] ) ) {
            return new WP_Error( 'sk_product_no_title_found', sprintf( __( 'Product title must be required', 'sk-core' ), 'product' ), [ 'status' => 404 ] );
        }

        $category_selection = sk_get_option( 'product_category_style', 'sk_selling', 'single' );

        if ( empty( $request['categories'] ) ) {
            return new WP_Error( 'sk_product_category', __( 'Category must be required', 'sk-core' ), [ 'status' => 404 ] );
        }

        if ( 'single' === $category_selection ) {
            if ( count( $request['categories'] ) > 1 ) {
                return new WP_Error( 'sk_product_category_no_more_one', __( 'You can not select more than category', 'sk-core' ), [ 'status' => 404 ] );
            }
        }
    ...
    public function validation_before_update_item( $request ) {
        ...
        if ( ! $object || 0 === $object->get_id() ) {
            return new WP_Error( "sk_rest_{$this->post_type}_invalid_id", __( 'Invalid ID.', 'sk-core' ), [ 'status' => 400 ] );
        }

        $product_author = (int) get_post_field( 'post_author', $object->get_id() );

        if ( $store_id !== $product_author && ! current_user_can( 'manage_options' ) ) {
            return new WP_Error( "sk_rest_{$this->post_type}_invalid_id", __( 'Sorry, you have no permission to do this. Since it\'s not your product.', 'sk-core' ), [ 'status' => 400 ] );
        }
    ...
    public function validation_before_delete_item( $request ) {
        ...
        if ( ! $object || 0 === $object->get_id() ) {
            return new WP_Error( "sk_rest_{$this->post_type}_invalid_id", __( 'Invalid ID.', 'sk-core' ), [ 'status' => 404 ] );
        }
    ...
    public function get_product_permissions_check() {
        return current_user_can( 'sk_view_product_menu' ) || current_user_can( 'manage_options' );
    }
    ...
    public function create_product_permissions_check() {
        return current_user_can( 'sk_add_product' ) || current_user_can( 'manage_options' );
    }
    ...
    public function get_single_product_permissions_check() {
        return current_user_can( 'skdar' ) || current_user_can( 'manage_options' );
    }
    ...
    public function update_product_permissions_check() {
        return current_user_can( 'sk_edit_product' ) || current_user_can( 'manage_options' );
    }
    ...
    public function delete_product_permissions_check() {
        return current_user_can( 'sk_delete_product' ) || current_user_can( 'manage_options' );
    }
    ...
    public function get_product_summary_permissions_check() {
        return current_user_can( 'sk_view_product_status_report' );
    }
    ...
    public function get_product_summary( $request ) {
        ...
        $data = [
            'post_counts'  => sk_count_posts( 'product', $seller_id ),
            'products_url' => sk_get_navigation_url( 'products' ),
        ];
    ...
    public function get_top_rated_product( $request ) {
        $result   = sk_get_top_rated_products( $request['per_page'], $request['seller_id'], $request['page'] );
        $data     = [];
        $response = [];
    ...
    public function get_best_selling_product( $request ) {
        $result   = sk_get_best_selling_products( $request['per_page'], $request['seller_id'], $request['page'] );
        $data     = [];
        $response = [];
    ...
    public function get_featured_product( $request ) {
        $result   = sk_get_featured_products( $request['per_page'], $request['seller_id'], $request['page'] );
        $data     = [];
        $response = [];
    ...
    public function get_latest_product( $request ) {
        $result   = sk_get_latest_products( $request['per_page'], $request['seller_id'], $request['page'] );
        $data     = [];
        $response = [];
    ...
    protected function prepare_data_for_response( $product, $request ) {
        ...
        $data      = [
            ...
            'row_actions'           => sk_product_get_row_action( $product->get_id(), false ),
        ];

        $response = rest_ensure_response( $data );
        $response->add_links( $this->prepare_links( $product, $request ) );

        return apply_filters( "sk_rest_prepare_{$this->post_type}_object", $response, $product, $request );
    }
    ...
    protected function prepare_object_for_database( $request, $creating = false ) {
        ...
        if ( 'variation' === $product->get_type() ) {
            return new WP_Error(
                "sk_rest_invalid_{$this->post_type}_id", __( 'To manipulate product variations you should use the /products/&lt;product_id&gt;/variations/&lt;id&gt; endpoint.', 'sk-core' ), [
                    'status' => 404,
                ]
            );
        }
        ...
        // Post status.
        $default_product_status = sk_get_default_product_status();
    ...
    protected function get_images( $product ) {
        ...
        if ( empty( $images ) ) {
            $images[] = [
                ...
                'name'              => __( 'Placeholder', 'sk-core' ),
                'alt'               => __( 'Placeholder', 'sk-core' ),
                'position'          => 0,
            ];
        }
        ...
        if ( ! empty( $images ) ) {
            ...
            foreach ( $images as $image ) {
                ...
                if ( 0 === $attachment_id && isset( $image['src'] ) ) {
                    ...
                    if ( is_wp_error( $upload ) ) {
                        if ( ! apply_filters( 'woocommerce_rest_suppress_image_upload_error', false, $upload, $product->get_id(), $images ) ) {
                            sk_log( 'Error uploading image: ' . $upload->get_error_message() );
                ...
                if ( $attachment_id && ! wp_attachment_is_image( $attachment_id ) ) {
                    ...
                    throw new WC_REST_Exception( 'woocommerce_product_invalid_image_id', sprintf( esc_html__( '#%s is an invalid image ID.', 'sk-core' ), esc_html( $attachment_id ) ), 400 );
                }
    ...
    public function get_multistep_categories() {
        ...
        $categories = apply_filters( 'sk_rest_product_categories', $categories_controller->get() );
    ...
    public function get_item_schema() {
        ...
        $schema         = [
            ...
            'properties' => [
                ...
                'slug'                  => [
                    'description' => __( 'Product slug.', 'sk-core' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                ],
                'permalink'             => [
                    'description' => __( 'Product URL.', 'sk-core' ),
                    'type'        => 'string',
                    'format'      => 'uri',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ...
                'date_created'          => [
                    'description' => __( "The date the product was created, in the site's timezone.", 'sk-core' ),
                    'type'        => 'date-time',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'date_created_gmt'      => [
                    'description' => __( 'The date the product was created, as GMT.', 'sk-core' ),
                    'type'        => 'date-time',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'date_modified'         => [
                    'description' => __( "The date the product was last modified, in the site's timezone.", 'sk-core' ),
                    'type'        => 'date-time',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'date_modified_gmt'     => [
                    'description' => __( 'The date the product was last modified, as GMT.', 'sk-core' ),
                    'type'        => 'date-time',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'type'                  => [
                    'description' => __( 'Product type.', 'sk-core' ),
                    'type'        => 'string',
                    'default'     => 'simple',
                ...
                'status'                => [
                    'description' => __( 'Product status (post status).', 'sk-core' ),
                    'type'        => 'string',
                    'default'     => 'publish',
                ...
                'featured'              => [
                    'description' => __( 'Featured product.', 'sk-core' ),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => [ 'view', 'edit' ],
                ],
                'catalog_visibility'    => [
                    'description' => __( 'Catalog visibility.', 'sk-core' ),
                    'type'        => 'string',
                    'default'     => 'visible',
                ...
                'description'           => [
                    'description' => __( 'Product description.', 'sk-core' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                ],
                'short_description'     => [
                    'description' => __( 'Product short description.', 'sk-core' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                ],
                'sku'                   => [
                ...
                'price'                 => [
                    'description' => __( 'Current product price.', 'sk-core' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'regular_price'         => [
                    'description' => __( 'Product regular price.', 'sk-core' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                ],
                'sale_price'            => [
                    'description' => __( 'Product sale price.', 'sk-core' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                ],
                'date_on_sale_from'     => [
                ...
                'price_html'            => [
                    'description' => __( 'Price formatted in HTML.', 'sk-core' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'on_sale'               => [
                    'description' => __( 'Shows if the product is on sale.', 'sk-core' ),
                    'type'        => 'boolean',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'purchasable'           => [
                    'description' => __( 'Shows if the product can be bought.', 'sk-core' ),
                    'type'        => 'boolean',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'total_sales'           => [
                    'description' => __( 'Amount of sales.', 'sk-core' ),
                    'type'        => 'integer',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'virtual'               => [
                    'description' => __( 'If the product is virtual.', 'sk-core' ),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => [ 'view', 'edit' ],
                ],
                'downloadable'          => [
                    'description' => __( 'If the product is downloadable.', 'sk-core' ),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => [ 'view', 'edit' ],
                ],
                ...
                'external_url'          => [
                    'description' => __( 'Product external URL. Only for external products.', 'sk-core' ),
                    'type'        => 'string',
                    'format'      => 'uri',
                    'context'     => [ 'view', 'edit' ],
                ],
                'button_text'           => [
                    'description' => __( 'Product external button text. Only for external products.', 'sk-core' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                ],
                'tax_status'            => [
                ...
                'manage_stock'          => [
                    'description' => __( 'Stock management at product level.', 'sk-core' ),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => [ 'view', 'edit' ],
                ],
                ...
                'in_stock'              => [
                    'description' => __( 'Controls whether or not the product is listed as "in stock" or "out of stock" on the frontend.', 'sk-core' ),
                    'type'        => 'boolean',
                    'default'     => true,
                    'context'     => [ 'view', 'edit' ],
                ],
                ...
                'backordered'           => [
                    'description' => __( 'Shows if the product is on backordered.', 'sk-core' ),
                    'type'        => 'boolean',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'sold_individually'     => [
                    'description' => __( 'Allow one item to be bought in a single order.', 'sk-core' ),
                    'type'        => 'boolean',
                    'default'     => false,
                    'context'     => [ 'view', 'edit' ],
                ],
                'weight'                => [
                    ...
                    'description' => sprintf( __( 'Product weight (%s).', 'sk-core' ), $weight_unit ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                ],
                'dimensions'            => [
                    'description' => __( 'Product dimensions.', 'sk-core' ),
                    'type'        => 'object',
                    'context'     => [ 'view', 'edit' ],
                    'properties'  => [
                        'length' => [
                            /* translators: %s: dimension unit */
                            'description' => sprintf( __( 'Product length (%s).', 'sk-core' ), $dimension_unit ),
                            'type'        => 'string',
                            'context'     => [ 'view', 'edit' ],
                        ],
                        'width'  => [
                            /* translators: %s: dimension unit */
                            'description' => sprintf( __( 'Product width (%s).', 'sk-core' ), $dimension_unit ),
                            'type'        => 'string',
                            'context'     => [ 'view', 'edit' ],
                        ],
                        'height' => [
                            /* translators: %s: dimension unit */
                            'description' => sprintf( __( 'Product height (%s).', 'sk-core' ), $dimension_unit ),
                            'type'        => 'string',
                            'context'     => [ 'view', 'edit' ],
                        ],
                    ],
                ...
                'shipping_required'     => [
                    'description' => __( 'Shows if the product need to be shipped.', 'sk-core' ),
                    'type'        => 'boolean',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'shipping_taxable'      => [
                    'description' => __( 'Shows whether or not the product shipping is taxable.', 'sk-core' ),
                    'type'        => 'boolean',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                ...
                'reviews_allowed'       => [
                    'description' => __( 'Allow reviews.', 'sk-core' ),
                    'type'        => 'boolean',
                    'default'     => true,
                    'context'     => [ 'view', 'edit' ],
                ],
                'average_rating'        => [
                    'description' => __( 'Reviews average rating.', 'sk-core' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'rating_count'          => [
                    'description' => __( 'Amount of reviews that the product have.', 'sk-core' ),
                    'type'        => 'integer',
                    'context'     => [ 'view', 'edit' ],
                    'readonly'    => true,
                ],
                'related_ids'           => [
                    'description' => __( 'List of related products IDs.', 'sk-core' ),
                    'type'        => 'array',
                    'items'       => [
                        'type' => 'integer',
                    ],
                ...
                'upsell_ids'            => [
                    'description' => __( 'List of up-sell products IDs.', 'sk-core' ),
                    'type'        => 'array',
                    'items'       => [
                        'type' => 'integer',
                    ],
                ...
                'cross_sell_ids'        => [
                    'description' => __( 'List of cross-sell products IDs.', 'sk-core' ),
                    'type'        => 'array',
                    'items'       => [
                        'type' => 'integer',
                    ],
                ...
                'parent_id'             => [
                    'description' => __( 'Product parent ID.', 'sk-core' ),
                    'type'        => 'integer',
                    'context'     => [ 'view', 'edit' ],
                ],
                'purchase_note'         => [
                    'description' => __( 'Optional note to send the customer after purchase.', 'sk-core' ),
                    'type'        => 'string',
                    'context'     => [ 'view', 'edit' ],
                ],
                'categories'            => [
                    'description' => __( 'List of categories.', 'sk-core' ),
                    'type'        => 'array',
                    'context'     => [ 'view', 'edit' ],
                    'items'       => [
                        'type'       => 'object',
                        'properties' => [
                            'id'   => [
                                'description' => __( 'Category ID.', 'sk-core' ),
                                'type'        => 'integer',
                                'context'     => [ 'view', 'edit' ],
                            ],
                            'name' => [
                                'description' => __( 'Category name.', 'sk-core' ),
                                'type'        => 'string',
                                'context'     => [ 'view', 'edit' ],
                                'readonly'    => true,
                            ],
                            'slug' => [
                                'description' => __( 'Category slug.', 'sk-core' ),
                                'type'        => 'string',
                                'context'     => [ 'view', 'edit' ],
                                'readonly'    => true,
                            ],
                ...
                'tags'                  => [
                    'description' => __( 'List of tags.', 'sk-core' ),
                    'type'        => 'array',
                    'context'     => [ 'view', 'edit' ],
                    'items'       => [
                        'type'       => 'object',
                        'properties' => [
                            'id'   => [
                                'description' => __( 'Tag ID.', 'sk-core' ),
                                'type'        => 'integer',
                                'context'     => [ 'view', 'edit' ],
                            ],
                            'name' => [
                                'description' => __( 'Tag name.', 'sk-core' ),
                                'type'        => 'string',
                                'context'     => [ 'view', 'edit' ],
                                'readonly'    => true,
                            ],
                            'slug' => [
                                'description' => __( 'Tag slug.', 'sk-core' ),
                                'type'        => 'string',
                                'context'     => [ 'view', 'edit' ],
                                'readonly'    => true,
                            ],
                ...
                'images'                => [
                    'description' => __( 'List of images.', 'sk-core' ),
                    'type'        => 'array',
                    'context'     => [ 'view', 'edit' ],
                    'items'       => [
                        'type'       => 'object',
                ...
                'variations'            => [
                    'description' => __( 'List of variations IDs.', 'sk-core' ),
                    'type'        => 'array',
                    'context'     => [ 'view', 'edit' ],
                    'items'       => [
                        'type' => 'integer',
                ...
                'grouped_products'      => [
                    'description' => __( 'List of grouped products ID.', 'sk-core' ),
                    'type'        => 'array',
                    'items'       => [
                        'type' => 'integer',
                    ],
