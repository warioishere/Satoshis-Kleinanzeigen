...
class ProductControllerV2 extends ProductController {
    ...
    public function __construct() {
        ...
        add_filter( 'sk_rest_product_listing_exclude_type', [ $this, 'reset_exclude_product_type' ], 99, 2 );
    }
    ...
    public function get_collection_params() {
        ...
        $params['author'] = array(
            'description'       => __( 'Products author id', 'sk-core' ),
            'type'              => 'integer',
        ...
        $params['post_status'] = array(
            'description'       => __( 'Product status publish, pending, draft etc.', 'sk-core' ),
            'type'              => 'array',
        ...
        $params['date'] = array(
            'description'       => __( 'Products publish month', 'sk-core' ),
            'type'              => 'string',
        ...
        $params['product_cat'] = array(
            'description'       => __( 'Products category.', 'sk-core' ),
            'type'              => 'integer',
            'sanitize_callback' => 'absint',
            'validate_callback' => 'rest_validate_request_arg',
            'required'          => false,
        ...
        $params['product_type'] = array(
            'description'       => __( 'Products type all, simple, variable, grouped product etc.', 'sk-core' ),
            'type'              => 'string',
            'sanitize_callback' => 'sanitize_text_field',
            'validate_callback' => 'rest_validate_request_arg',
            'required'          => false,
        ...
        $params['stock_status'] = array(
            'description'       => __( 'Products stock status in stock or out of stock.', 'sk-core' ),
            'type'              => 'string',
            'sanitize_callback' => 'sanitize_text_field',
            'validate_callback' => 'rest_validate_request_arg',
            'required'          => false,
        ...
        $params['filter_by_other'] = array(
            'description'       => __( 'Best selling, featured products etc.', 'sk-core' ),
            'type'              => 'string',
            'sanitize_callback' => 'sanitize_text_field',
            'validate_callback' => 'rest_validate_request_arg',
            'required'          => false,
        ...
        $params['include'] = array(
            'description'       => __( 'Limit result set to specific ids.', 'sk-core' ),
            'type'              => 'array',
            'items'             => array(
                'type' => 'integer',
            ),
        ...
        $params['exclude'] = array(
            'description'       => __( 'Ensure result set excludes specific IDs.', 'sk-core' ),
            'type'              => 'array',
            'items'             => array(
                'type' => 'integer',
            ),
        ...
        $params['include_variations'] = array(
            'description' => __( 'If true matched product variations will be added to the collection', 'sk-core' ),
            'type'        => 'boolean',
            'default'     => false,
        );
    ...
    public function get_product_filter_by_data() {
        ...
        $months = sk_get_products_listing_months_for_vendor( sk_get_current_user_id() );

        foreach ( $months as $key => $arc_row ) {
            $month = zeroise( $arc_row->month, 2 );

            // translators: 1: month name, 2: 4-digit year
            $months [ $key ]->title = sprintf( esc_html__( '%1$s %2$d', 'sk-core' ), esc_html( $wp_locale->get_month( $month ) ), esc_html( $arc_row->year ) );
        }
        return [
            'allDates'   => $months,
        ];
    ...
    protected function prepare_objects_query( $request ) {
            ...
            array(
                ...
                'tax_query'      => array( // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_tax_query
                    array(
                        ...
                        'field'    => 'slug',
                        'terms'    => apply_filters( 'sk_rest_product_listing_exclude_type', array(), $request ),
                        'operator' => 'NOT IN',
                    ),
                ),
            )
        ...
        return apply_filters( 'sk_rest_pre_product_listing_args', $args, $request );
    }
    ...
    public function get_filter_data_schema() {
        return array(
            ...
            'properties' => array(
                'allDates'  => array(
                    'description' => esc_html__( 'All product created months.', 'sk-core' ),
                    'type'        => 'array',
                    'readonly'    => true,
                    'items' => array(
                        'type'   => 'object',
                        'properties' => array(
                            'year' => array(
                                'description' => esc_html__( 'Product publish year.', 'sk-core' ),
                                'type'        => 'integer',
                            ),
                            'month' => array(
                                'description' => esc_html__( 'Product publish month.', 'sk-core' ),
                                'type'        => 'integer',
                            ),
                            'title' => array(
                                'description' => esc_html__( 'Product publish month and year full title.', 'sk-core' ),
                                'type'        => 'string',
                            ),
                        ),
                    ),
    ...
    public function reset_exclude_product_type( $exclude_product_type, $request ) {
        $exclude_product_type = apply_filters( 'sk_product_listing_exclude_type', $exclude_product_type );
        $product_type         = $request->get_param( 'product_type' );
