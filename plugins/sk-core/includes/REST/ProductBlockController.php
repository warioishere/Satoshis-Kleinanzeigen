...
class ProductBlockController extends ProductController {
    ...
    public function get_item( $request ) {
        ...
        if ( ! $product ) {
            return new WP_Error( 'product_not_found', __( 'Product not found', 'sk-core' ), array( 'status' => 404 ) );
        }

        $context = ! empty( $request['context'] ) ? $request['context'] : 'view';

        // Get chosen product categories for sk multi-step category, if not found then generate it and return it.
        ...
        return apply_filters(
            'sk_rest_get_product_block_data',
            [
                'general' => [
