...
class ProductAttributeController extends WC_REST_Product_Attributes_V1_Controller {
    ...
    public function create_item_permissions_check( $request ) {
        return current_user_can( 'sk_add_product' );
    }
    ...
    public function get_item_permissions_check( $request ) {
        if ( ! $this->get_taxonomy( $request ) ) {
            return new WP_Error( 'sk_rest_taxonomy_invalid', __( 'Resource does not exist.', 'sk-core' ), array( 'status' => 404 ) );
        }
    ...
    public function update_item_permissions_check( $request ) {
        ...
        return current_user_can( 'sk_edit_product' );
    }
    ...
    public function delete_item_permissions_check( $request ) {
        if ( ! $this->get_taxonomy( $request ) ) {
            return new WP_Error( 'woocommerce_rest_taxonomy_invalid', __( 'Resource does not exist.', 'sk-core' ), array( 'status' => 404 ) );
        }

        return current_user_can( 'sk_delete_product' );
    }
    ...
    public function get_product_update_collection_params() {
        return [
            'attributes'  => [
                'description' => __( 'Attribute options.', 'sk-core' ),
                'type'        => 'array',
                'required'    => true,
                'context'     => [ 'edit' ],
                'items'       => [
                    ...
                    'properties' => [
                        ...
                        'visible' => [
                            'description'       => __( 'Attribute visible in product list page or not.', 'sk-core' ),
                            'type'              => 'boolean',
                            'default'           => false,
                            'context'           => [ 'edit' ],
                        ...
                        'variation' => [
                            'description'       => __( 'Attribute is for variation or not.', 'sk-core' ),
                            'type'              => 'boolean',
                            'default'           => false,
                            'context'           => [ 'edit' ],
                            'sanitize_callback' => 'boolval',
                        ...
                        'options' => [
                            'description' => __( 'Attribute values.', 'sk-core' ),
                            'type'        => 'array',
                            'required'    => true,
                            'context'     => [ 'edit' ],
                        ],
    ...
    public function update_product_attribute( $request ) {
        ...
        if ( empty( $product_id ) ) {
            return new WP_Error( 'invalid_product_id', __( 'Invalid product id.', 'sk-core' ), [ 'status' => 400 ] );
        }

        $product = wc_get_product( $product_id );

        if ( empty( $product ) ) {
            return new WP_Error( 'no_product_found', __( 'No product found.', 'sk-core' ), [ 'status' => 404 ] );
        }
        ...
        if ( ! $is_saved ) {
            return new WP_Error( 'product_bulk_attribute_terms_saved_failed', __( 'Failed to save product bulk attribute and terms. Please try again later.', 'sk-core' ), [ 'status' => 400 ] );
        }
    ...
    public function update_product_default_attribute( $request ) {
        ...
        if ( ! $is_saved ) {
            return new WP_Error( 'product_default_attribute_saved_failed', __( 'Failed to save product default attribute and terms. Please try again later.', 'sk-core' ), [ 'status' => 400 ] );
        }
