...
/**
* Base REST Controller for sk
*
*
*/
abstract class SkRESTController extends WP_REST_Controller {
    ...
    protected function get_object( $id ) {
        ...
        return new WP_Error( 'invalid-method', sprintf( __( "Method '%s' not implemented. Must be overridden in subclass.", 'sk-core' ), __METHOD__ ), array( 'status' => 405 ) );
    }
    ...
    public function create_item( $request ) {
        ...
        try {
             ...
             */
            do_action( "sk_rest_insert_{$this->post_type}_object", $object, $request, true );
    ...
    public function update_item( $request ) {
        ...
        try {
             ...
             * @param boolean         $creating  True when creating object, false when updating.
             */
            do_action( "sk_rest_insert_{$this->post_type}_object", $object, $request, false );
    ...
    public function delete_item( $request ) {
        ...
        if ( ! $result ) {
            ...
            return new WP_Error( 'sk_rest_cannot_delete', sprintf( __( 'The %s cannot be deleted.', 'sk-core' ), $this->post_type ), array( 'status' => 500 ) );
        }
     ...
     *
     * @param  SK_Data         $object  Object data.
     * @param  WP_REST_Request $request Request object.
    ...
    protected function prepare_objects_query( $request ) {
        ...
        $args['author']              = ! isset( $request['id'] ) ? sk_get_current_user_id() : $request['id'];
         ...
         */
        $args = apply_filters( "sk_rest_{$this->post_type}_object_query", $args, $request );
    ...
    protected function prepare_items_query( $prepared_args = array(), $request = null ) {
        ...
        foreach ( $valid_vars as $var => $index ) {
            if ( isset( $prepared_args[ $var ] ) ) {
                $query_args[ $var ] = apply_filters( "sk_rest_query_var_{$var}", $prepared_args[ $var ] );
            }
        }
    ...
    protected function get_allowed_query_vars() {
        ...
        $valid_vars = apply_filters( 'sk_rest_query_vars', $valid_vars );
    ...
    public function update_post_author_if_needed( WP_REST_Request $request, int $object_id ) {
        $author_id = sk_get_current_user_id();
        ...
        if ( current_user_can( 'manage_options' ) ) {
            ...
            $author_id = ( ! empty( $post_author ) && $author->exists() && user_can( $author->ID, 'sk_add_product' ) ) ? $author->ID : $author_id;
        }
