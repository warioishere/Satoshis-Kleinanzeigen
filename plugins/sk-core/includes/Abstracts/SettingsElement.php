...
abstract class SettingsElement {
	...
	public function set_children( array $children ): SettingsElement {
        if ( ! $this->is_support_children() ) {
            ...
            throw new Exception( sprintf( esc_html__( 'Settings %s Does not support adding any children.', 'sk-core' ), esc_html( $this->get_type() ) ) );
        }
	...
	public function remove( SettingsElement $element ): SettingsElement {
		if ( ! $this->is_support_children() ) {
			// translators: %s is Settings element type.
			throw new Exception( sprintf( esc_html__( 'Settings %s Does not support removing any children.', 'sk-core' ), esc_html( $this->get_type() ) ) );
		}
