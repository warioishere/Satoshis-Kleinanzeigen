...
abstract class ProductStatusChanger {
    ...
    public function __construct() {
        add_action( 'sk_product_status_changer_async', [ $this, 'process_background_task' ] );
        add_action( 'sk_clear_product_cache_async', [ $this, 'clear_product_cache' ] );
    }
    ...
    public function set_per_page( $per_page ) {
        $this->per_page = apply_filters( 'sk_product_status_changer_per_page', $per_page );
    }
    ...
    public function reset() {
        $this->vendor_id = 0;
        $this->page      = 1;
        $this->per_page  = apply_filters( 'sk_product_status_changer_per_page', 100 );
    ...
    public function add_to_queue( $task_type = 'change_status', $status = null ) {
        ...
        while ( $products = $this->get_products() ) { // phpcs:ignore
            ...
            if ( empty( $products ) ) {
                ...
                // clear product cache.
                WC()->queue()->add( 'sk_clear_product_cache_async', [ $args ] );
                break;
            }
            ...
            WC()->queue()->add( 'sk_product_status_changer_async', [ $args ] );

            // increment current page.
            $this->increment_current_page();
        }

        do_action( 'sk_product_status_changer_after_queued', $this );
    }
    ...
    private function change_status( $product_id, $status ) {
        ...
        $product->update_meta_data( '_sk_previous_status', $current_status );

        do_action( 'sk_product_change_status_before_save', $product, $status );

        // save product
        $product->save();

        do_action( 'sk_product_change_status_after_save', $product, $status );
    }
    ...
    private function revert( $product_id ) {
        ...
        $previous_status = $product->get_meta( '_sk_previous_status' );
        if ( ! $previous_status ) {
        ...
        $product->delete_meta_data( '_sk_previous_status' );

        do_action( 'sk_product_status_revert_before_save', $product, $previous_status );

        // save product
        $product->save();

        do_action( 'sk_product_status_revert_after_save', $product, $previous_status );
    }
}
