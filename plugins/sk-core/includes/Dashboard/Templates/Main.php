...
class Main {
    ...
    public function __construct() {
        add_action( 'sk_dashboard_content_before', [ self::class, 'dashboard_side_navigation' ] );
        add_filter( 'sk_vendor_dashboard_menu_title', [ $this, 'add_notification_count' ], 10, 2 );
    }
    ...
    public static function dashboard_side_navigation() {
        ...
        sk_get_template_part( 'global/dashboard-nav', '', [ 'active_menu' => apply_filters( 'sk_dashboard_nav_active', $active_menu, $request, $active ) ] );
    }
    ...
    public function add_notification_count( string $menu_title, array $menu_details ): string {
        if ( ! empty( $menu_details['counts'] ) ) {
            ...
            $notification_counts = apply_filters(
                'sk_vendor_dashboard_menu_notification_count',
