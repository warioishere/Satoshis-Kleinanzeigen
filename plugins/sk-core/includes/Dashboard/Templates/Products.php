...
class Products {
    ...
    public function __construct() {
        add_action( 'sk_render_product_listing_template', [ $this, 'render_product_listing_template' ], 11 );
     ...
     *
     *
     * @uses apply_filters() Calls 'sk_hide_inventory_template' to allow plugins
    ...
    public static function load_inventory_template( $post, $post_id ) {
        $hide_inventory = (bool) apply_filters( 'sk_hide_inventory_template', false, $post_id );
        if ( $hide_inventory ) {
            return;
        }
    ...
    public static function load_others_template( $post, $post_id ) {
        ...
        // set new post status
        $post_status = sk_get_default_product_status( sk_get_current_user_id() );
        ...
        sk_get_template_part(
            'products/others', '', [
                'post_id'            => $post_id,
                'post'               => $post,
                'post_status'        => apply_filters( 'sk_post_edit_default_status', $post_status, $product ),
    ...
    public function render_new_product_template( $query_vars ) {
        if ( isset( $query_vars['new-product'] ) && ! sk()->is_pro_exists() ) {
    ...
    public function load_product_edit_template() {
        if ( current_user_can( 'sk_edit_product' ) ) {
            sk_get_template_part( 'products/edit-product-single' );
        } else {
            sk_get_template_part(
                'global/sk-error', '', [
                    'deleted' => false,
    ...
    public function render_product_edit_page_for_email() {
        ...
        $product_edit_url = remove_query_arg( '_view_mode', sk_get_current_page_url() );
        $product_edit_url = add_query_arg( '_sk_edit_product_nonce', wp_create_nonce( 'sk_edit_product_nonce' ), $product_edit_url );
    ...
    public function render_product_listing_template( $action ) {
        ...
        $bulk_statuses = apply_filters(
            'sk_bulk_product_statuses', [
                ...
                'delete' => __( 'Delete Permanently', 'sk-core' ),
            ]
        );
    ...
    public function handle_product_add() {
        ...
        if ( ! isset( $_POST['sk_add_new_product_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['sk_add_new_product_nonce'] ), 'sk_add_new_product' ) ) {
            return;
        }
        ...
        $errors             = [];
        self::$product_cat  = - 1;
        self::$post_content = __( 'Details of your product ...', 'sk-core' );
        ...
        if ( isset( $postdata['add_product'] ) ) {
            ...
            if ( empty( $post_title ) ) {
                $errors[] = __( 'Please enter product title', 'sk-core' );
            }

            if ( ! isset( $postdata['chosen_product_cat'] ) ) {
                if ( Helper::product_category_selection_is_single() ) {
                    if ( absint( $postdata['product_cat'] ) < 0 ) {
                        $errors[] = __( 'Please select a category', 'sk-core' );
                    }
                } elseif ( ! isset( $postdata['product_cat'] ) || empty( $postdata['product_cat'] ) ) {
                        $errors[] = __( 'Please select at least one category', 'sk-core' );
                }
            } elseif ( empty( $postdata['chosen_product_cat'] ) ) {
                $errors[] = __( 'Please select a category', 'sk-core' );
            }

            self::$errors = apply_filters( 'sk_can_add_product', $errors );

            if ( ! self::$errors ) {
                $timenow        = sk_current_datetime()->setTimezone( new \DateTimeZone( 'UTC' ) );
                $product_status = sk_get_default_product_status( sk_get_current_user_id() );
                $post_data      = apply_filters(
                    'sk_insert_product_post_data', [
                        'post_type'         => 'product',
                ...
                if ( $product_id ) {
                    ...
                    do_action( 'sk_new_product_added', $product_id, $postdata );

                    if ( current_user_can( 'sk_edit_product' ) ) {
                        $redirect = sk_edit_product_url( $product_id );
                    } else {
                        $redirect = sk_get_navigation_url( 'products' );
                    }
                    ...
                    wp_safe_redirect( apply_filters( 'sk_add_new_product_redirect', $redirect, $product_id ) );
                    exit;
                }
            }
        }
    ...
    public function handle_product_update() {
        ...
        if ( ! isset( $_POST['sk_update_product'] ) ) {
            return;
        }

        if ( ! isset( $_POST['sk_edit_product_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['sk_edit_product_nonce'] ), 'sk_edit_product' ) ) {
            return;
        }
        ...
        $post_id = isset( $_POST['sk_product_id'] ) ? absint( $_POST['sk_product_id'] ) : 0;
        if ( ! $post_id ) {
            $errors[] = __( 'No product found!', 'sk-core' );
        }
        ...
        if ( ! sk_is_product_author( $post_id ) ) {
            $errors[] = __( 'I swear this is not your product!', 'sk-core' );
        }
        ...
        if ( isset( $_POST['product_tag'] ) ) {
            ...
            if ( $maximum_tags_select_length !== - 1 && $tag_count > $maximum_tags_select_length ) {
                ...
                $errors[] = sprintf( __( 'You can only select %s tags', 'sk-core' ), number_format_i18n( $maximum_tags_select_length ) );
            }
        }

        self::$errors = apply_filters( 'sk_can_edit_product', $errors );
        ...
        if ( $is_new_product ) {
            do_action( 'sk_new_product_added', $post_id, $postdata );
        } else {
            do_action( 'sk_product_updated', $post_id, $postdata );
        }

        $redirect = apply_filters( 'sk_add_new_product_redirect', sk_edit_product_url( $post_id ), $post_id );

        // if any error inside sk_process_product_meta function
    ...
    public function handle_delete_product() {
        ...
        if ( ! isset( $_GET['action'] ) || $_GET['action'] !== 'sk-delete-product' ) {
            return;
        }

        if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_GET['_wpnonce'] ), 'sk-delete-product' ) ) {
            wp_safe_redirect( add_query_arg( [ 'message' => 'error' ], sk_get_navigation_url( 'products' ) ) );
            exit;
        }
        ...
        do_action( 'sk_product_delete', $product_id );

        sk()->product->delete( $product_id, true );

        do_action( 'sk_product_deleted', $product_id );

        $redirect = apply_filters( 'sk_add_new_product_redirect', sk_get_navigation_url( 'products' ), '' );
