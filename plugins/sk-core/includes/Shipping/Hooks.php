...
class Hooks {
    ...
    public function __construct() {
        if ( ! sk()->is_pro_exists() ) {
            // todo: permanently move below hooks from sk pro after a couple of release, we are moving shipping split feature to free version.
    ...
    public function change_shipping_pack_name( $title, $i, $package ) {
        ...
        $shipping_label = sprintf( '%s %s', __( 'Shipping: ', 'sk-core' ), $vendor->get_shop_name() );

        return apply_filters( 'sk_shipping_package_name', $shipping_label, $i, $package, $vendor );
    }
