...
class StoreCategory {
    ...
    public function __construct() {
        ...
        add_filter( 'sk_settings_general_vendor_store_options', array( $this, 'add_admin_settings' ) );
        add_action( 'sk_after_saving_settings', array( $this, 'set_default_category' ), 10, 2 );
        add_filter( 'sk_admin_localize_script', array( $this, 'add_localized_data' ) );
        add_filter( 'sk_admin_dashboard_pages_settings', array( $this, 'add_localized_data' ) );
        add_filter( 'sk_localized_args', array( $this, 'set_localized_data' ) );

        if ( sk_is_store_categories_feature_on() ) {
            // Store category field — now inlined directly in store-form.php template
            // add_action( 'sk_settings_after_store_name', array( $this, 'add_store_category_option' ) );
            add_action( 'sk_seller_wizard_store_setup_after_address_field', array( $this, 'seller_wizard_add_store_category_option' ) );
            add_action( 'sk_new_seller_created', array( $this, 'after_sk_new_seller_created' ) );
            add_action( 'sk_new_vendor', array( $this, 'after_sk_new_seller_created' ), 10, 1 );
            add_action( 'user_register', array( $this, 'add_store_category_to_new_seller' ), 10, 2 );
            add_action( 'sk_store_profile_saved', array( $this, 'after_store_profile_saved' ) );
            add_action( 'sk_seller_wizard_store_field_save', array( $this, 'after_seller_wizard_store_field_save' ) );
            add_filter( 'sk_vendor_shop_data', array( $this, 'add_store_categories_in_vendor_shop_data' ), 10, 2 );
            ...
            add_filter( 'sk_rest_api_store_collection_params', [ $this, 'add_store_category_rest_collection_param' ] );
        }
    }
    ...
    private function register_taxonomy() {
        register_taxonomy(
            ...
            array(
                ...
                'show_in_rest' => sk_is_store_categories_feature_on(),
            )
        );
    }
     ...
     *
     *
     * @param array $sk_settings_fields
     *
     * @return array
     */
    ...
    public function add_localized_data( $data ) {
        $data['store_category_type'] = sk_get_option( 'store_category_type', 'sk_general', 'none' );
        return $data;
    }
    ...
    public function add_store_category_option( $current_user, $args = array(), $template_name = 'settings/store-form-categories' ) {
        ...
        sk_get_template_part( $template_name, '', $args );
    }
     ...
     * @return void
     */
    public function after_sk_new_seller_created( $user_id ) {
        sk_set_store_categories( $user_id );
    }
    ...
    public function after_store_profile_saved( $store_id ) {
        ...
        $store_categories = ! empty( $get_postdata['sk_store_categories'] ) ? $get_postdata['sk_store_categories'] : null;

        if ( $store_categories ) {
            sk_set_store_categories( $store_id, $store_categories );
        }
    }
    ...
    public function after_seller_wizard_store_field_save( $wizard ) {
        ...
        sk_set_store_categories( $wizard->store_id, $store_categories );
    }

    /**
     * Add store categories in \SK\Core\Vendor\Vendor shop_data
     *
     *
    ...
    public function add_store_categories_in_vendor_shop_data( $shop_info, $vendor ) {
        ...
        if ( empty( $store_categories ) ) {
            $default_category_id   = sk_get_default_store_category_id();
            ...
            if ( is_wp_error( $default_category_term ) ) {
                ...
                sk_set_store_categories( $vendor->get_id(), [ $default_category_id ] );
            }
        }

        $shop_info['categories'] = apply_filters( 'sk_get_store_categories_in_vendor', $store_categories );

        return $shop_info;
    }

    /**
     * Add store categories in \SK\Core\Vendor\Vendor to_array data
     *
     *
     * @param array        $data
     * @param \SK\Core\Vendor\Vendor $vendor
    ...
    public function add_store_category_to_new_seller( $user_id, $userdata ) {
        ...
        // Set default store category to the user.
        sk_set_store_categories( $user_id, [ $default_category_id ] );
    }
     ...
     *
     *
     * @param \SK\Core\Vendor\Vendor    $store
    ...
    /**
     * Add tax_query arg in WP_User_Query used in sk()->vendor->get_vendors()
     *
     *
     * @param array $args
     *
