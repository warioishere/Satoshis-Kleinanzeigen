...
/**
 * Core Class for SK Main functionality
 *
 *
 */
class Core {
    ...
    public function hide_others_uploads( $where ) {
        ...
        if ( ( $pagenow === 'upload.php' || $pagenow === 'media-upload.php' ) && current_user_can( 'skdar' ) ) {
    ...
    /**
     * Add body class for sk-dashboard
     *
     *
     * @param array $classes
     */
    public function add_dashboard_template_class( $classes ) {
        $page_id = sk_get_option( 'dashboard', 'sk_pages' );
        ...
        $classes[] = 'sk-theme-' . get_option( 'template' );
    ...
    public function wp_title( $title, $sep ) {
        ...
        if ( sk_is_store_page() ) {
            ...
            if ( $paged >= 2 || $page >= 2 ) {
                ...
                $title = "$title $sep " . sprintf( __( 'Page %1$s', 'sk-core' ), max( $paged, $page ) );
            }
    ...
    public function redirect_if_not_logged_seller() {
        ...
        if ( is_page( $page_id ) || apply_filters( 'sk_force_page_redirect', false, $page_id ) ) {
            sk_redirect_login();
