        ...
        try {
            ...
            while ( microtime( true ) < $this->deadline ) {
                ...
                if ( 'OK' === $data[0] && null !== $this->auth_id && ( $data[1] ?? '' ) === $this->auth_id ) {
                    ...
                    // The relay took no auth (relay.damus.io refuses every
                    // answer while its own address is not configured): a
                    // refused request is never answered, nothing to wait for.
                    if ( ! $this->authed ) {
