...
use SK\Modules\Reputation\RelayReader;
...
sk_check_eq( RelayReader::verified( $good, 3, [ $kp['pub'] ] ), true, 'verified(): signed event of the right kind and author' );
sk_check_eq( RelayReader::verified( $good, 0 ), false, 'verified(): wrong kind rejected' );
sk_check_eq( RelayReader::verified( $good, 3, [ str_repeat( 'a', 64 ) ] ), false, 'verified(): wrong author rejected' );
$tampered = $good; $tampered['content'] .= 'x';
sk_check_eq( RelayReader::verified( $tampered ), false, 'verified(): tampered content rejected' );

// ── Dead relays: nothing answers, nothing is published ────────────────────
$GLOBALS['sk_test_relay'] = "ws://127.0.0.1:1\nws://127.0.0.1:2";
$answered = null;
$latest   = RelayReader::latest( 3, [ $kp['pub'] ], $answered );
...
if ( $mirrors ) {
    ...
    sk_check( $ev && RelayReader::verified( $ev, 3 ), 'published list carries a valid signature' );
}
